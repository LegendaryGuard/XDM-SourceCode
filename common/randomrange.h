...
Copyright (C) 2011 Uncle Mike
  ...
  XDM: updated, but needs to be checked
*/
...
// NOTE: Default constructor must set all values to zero (particle code relies on it)
class RandomRange
{
public:
	RandomRange(void) { m_flMin = m_flMax = 0.0f; m_bDefined = false; }
	RandomRange(const float &fValue) { m_flMin = m_flMax = fValue; m_bDefined = true; }
	RandomRange(const float &fMin, const float &fMax) { m_flMin = fMin; m_flMax = fMax; m_bDefined = true; }
