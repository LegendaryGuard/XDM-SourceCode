...
int ReadBMP(const char *szFile, byte **ppbBits, byte **pRGBPalette256, int *width, int *height)
{
	int i, rc = 0;
	FILE *pfile = NULL;
	...
	ULONG cbBmpBits = 0;
	byte *pbBmpBits = NULL;
	byte *pbReadBits = NULL;// allocates
	byte *pbPal = NULL;// allocates
	ULONG cbPalBytes = 0;
	ULONG biTrueWidth = 0;

	if (szFile == NULL || *ppbBits != NULL || pRGBPalette256 == NULL)
	...
	*pRGBPalette256 = pbPal;
	...
	{
		*pbPal++ = rgrgbPalette[i].rgbRed;
		*pbPal++ = rgrgbPalette[i].rgbGreen;
		*pbPal++ = rgrgbPalette[i].rgbBlue;
/*		pRGBPalette256[i][0] = rgrgbPalette[i].rgbRed;
		pRGBPalette256[i][1] = rgrgbPalette[i].rgbGreen;
		pRGBPalette256[i][2] = rgrgbPalette[i].rgbBlue;*/
	}
	for (i = bmih.biClrUsed; i < 256; ++i)// unused entires 0,0,0
	{
		...
		*pbPal++ = 0;
		*pbPal++ = 0;
/*		pRGBPalette256[i][0] = 0;
		pRGBPalette256[i][1] = 0;
		pRGBPalette256[i][2] = 0;*/
	}
	...
	if (width) *width = (WORD)bmih.biWidth;
	if (height) *height = (WORD)bmih.biHeight;
	*ppbBits = pbBmpBits;
