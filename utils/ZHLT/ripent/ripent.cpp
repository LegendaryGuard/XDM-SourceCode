...
// ~X~
static void WriteTextureList(const char* const name)
{
	char outputfilename[_MAX_PATH];
	FILE *outputfile;
#ifdef ZHLT_DEFAULTEXTENSION_FIX
	safe_snprintf(outputfilename, _MAX_PATH, "%s.lst", name);
#else
    safe_strncpy(outputfilename, name, _MAX_PATH);
    StripExtension(outputfilename);
    DefaultExtension(outputfilename, ".lst");
#endif
    _unlink(outputfilename);
	outputfile = SafeOpenWrite(outputfilename);
	Log("\nWriting %s.\n", outputfilename);

	int ofs;
	for (int i = 0; i < ((dmiptexlump_t *)g_dtexdata)->nummiptex; i++)
	{
		ofs = ((dmiptexlump_t *)g_dtexdata)->dataofs[i];
		if (ofs >= 0)
		{
			fputs(((miptex_t *)(g_dtexdata+ofs))->name, outputfile);	// fail! SafeWrite(outputfile, ((miptex_t *)(g_dtexdata+ofs))->name, 16);// fprintf()?
			fputc('\n', outputfile);
		}
	}
	fclose(outputfile);
...
static void     Usage(void)
{
	...
	Log("    -texturelist    : Export list of used textures\n\n");// ~X~
#endif
#ifdef ZHLT_64BIT_FIX
...
int             main(int argc, char** argv)
{
    ...
    for (i = 1; i < argc; i++)
    {
		...
		else if (!strcasecmp(argv[i], "-texturelist"))// ~X~
		{
			g_texturemode = hl_exportlist;
		}
