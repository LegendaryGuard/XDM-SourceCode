...
TYPEDESCRIPTION	CBaseDoor::m_SaveData[] =
{
	...
	DEFINE_FIELD(CBaseDoor, m_bDontDraw, FIELD_CHARACTER),// XDM3038c
};
...
void CBaseDoor::KeyValue(KeyValueData *pkvd)
{
	...
	{
		pev->scale = (float)atof(pkvd->szValue) * (1.0f/8.0f);// engine reads this
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "dontdraw"))// XDM3037: flags
	{
		m_bDontDraw = atoi(pkvd->szValue);// 0-draw always, 1-dd opn, 2-dd cls
		// XDM3037: 'hot' update now
		if (m_bDontDraw == 0)
		{
			ClearBits(pev->effects, EF_NODRAW);
		}
		else if (FBitSet(m_bDontDraw, DOOR_NODRAW_CLOSED))
		{
		...
		else if (FBitSet(m_bDontDraw, DOOR_NODRAW_OPEN))
		{
...
void CBaseDoor::Spawn(void)
{
	...
	{// special contents
		pev->solid		= SOLID_NOT;// XDM: we can't track down how water touches things (water physics disappear when any other solid property set)
	...
	if (m_bDontDraw != 0 && m_flLip != 0.0f)
		conprintf(1, "Warning! %s %s has both lip value (%g) and 'don't draw' flag (%d) set!\n", STRING(pev->classname), STRING(pev->targetname), m_flLip, m_bDontDraw);
	...
	{	// swap pos1 and pos2, put door at pos2
		...
		if (FBitSet(m_bDontDraw, DOOR_NODRAW_OPEN))// XDM
			SetBits(pev->effects, EF_NODRAW);
	...
	{
		if (FBitSet(m_bDontDraw, DOOR_NODRAW_CLOSED))// XDM
			SetBits(pev->effects, EF_NODRAW);
...
void CBaseDoor::Precache(void)
{
	...
	if (m_bLockedSound)
	{
		...
		PRECACHE_SOUND(STRINGV(m_ls.sLockedSound));// XDM3038: allocation warning!
	}
	...
	if (m_bUnlockedSound)
	{
		...
		PRECACHE_SOUND(STRINGV(m_ls.sUnlockedSound));// XDM3038
	}
...
void CBaseDoor::DoorTouchDmgOnly(CBaseEntity *pOther)// XDM
{
	...
	{
		//conprintf(0, "CBaseDoor::DoorTouchDmgOnly()\n");
...
void CBaseDoor::DoorTouch(CBaseEntity *pOther)
{
	...
	if (IsLockedByMaster(pOther) ||
		!FStringNull(pev->targetname) && !FBitSet(pev->spawnflags, SF_DOOR_FORCETOUCHABLE))// XDM3037
	{
...
{
	//conprintf(1, "Door %s: m_toggle_state %d, m_flWait %f\n", STRING(pev->targetname), m_toggle_state, m_flWait);
...
// emit door moving and stop sounds on CHAN_STATIC so that the multicast doesn't filter them out and leave a client stuck with looping door sounds!
void CBaseDoor::StartSound(void)
{
	if (!FBitSet(pev->spawnflags, SF_DOOR_SILENT))
	{
		if (!FStringNull(pev->noiseMoving))
			EMIT_SOUND(edict(), DOOR_SNDCHAN_MOVING, STRINGV(pev->noiseMoving), VOL_NORM, ATTN_NORM);

		CSoundEnt::InsertSound(bits_SOUND_WORLD, pev->origin, DOOR_VSOUND_VOLUME, 2.0f);// XDM3038c
	}
	pev->frame = 1;// XDM3037
}
...
void CBaseDoor::StopSound(void)
{
	...
	{
		if (!FStringNull(pev->noiseMoving))
			STOP_SOUND(edict(), DOOR_SNDCHAN_MOVING, STRINGV(pev->noiseMoving));
		if (!FStringNull(pev->noiseArrived))
			EMIT_SOUND(edict(), DOOR_SNDCHAN_ARRIVED, STRINGV(pev->noiseArrived), VOL_NORM, ATTN_NORM);

		CSoundEnt::InsertSound(bits_SOUND_WORLD, pev->origin, DOOR_VSOUND_VOLUME, 1.0f);// XDM3038c
	}
	pev->frame = 0;// XDM3037
}
...
void CBaseDoor::DoorGoUp(void)
{
	...
	//if (FBitSet(pev->effects, EF_NODRAW))// XDM
		ClearBits(pev->effects, EF_NODRAW);// XDM3038a
	...
	{
		float	sign = 1.0;
		if (m_hActivator != NULL && m_hActivator->IsHuman())
		{
	...
	pev->scale *= 2.0f;// XDM3037: more waves while moving
}
...
void CBaseDoor::DoorHitTop(void)
{
	...
	pev->scale *= 0.5f;// XDM3037: restore wave height
	...
	if (!FStringNull(pev->netname) && FBitSet(pev->spawnflags, SF_DOOR_START_OPEN))
...
void CBaseDoor::Blocked(CBaseEntity *pOther)
{
	...
	if (m_flWait >= 0)
	{
		if (!FBitSet(pev->spawnflags, SF_DOOR_SILENT) && !FStringNull(pev->noiseMoving))// XDM
			STOP_SOUND(edict(), DOOR_SNDCHAN_MOVING, STRINGV(pev->noiseMoving));
	...
	if (!FStringNull(pev->targetname))
	{
		...
		for (;;)
		{
			...
			if (VARS(pentTarget) != pev)
			{
				...
				{
					pDoor = (CBaseDoor *)CBaseDoor::Instance(pentTarget);//GetClassPtr((CBaseDoor *)VARS(pentTarget));
					if (pDoor->m_flWait >= 0)
					{
						if (pDoor->pev->velocity == pev->velocity && pDoor->pev->avelocity == pev->velocity)
						{
							...
							{// set angles to realign rotating doors
								...
								pDoor->pev->avelocity.Clear();
							}
							else
							{// set origin to realign normal doors
								pDoor->pev->origin = pev->origin;
								pDoor->pev->velocity.Clear();// stop!
							}
						}

						if (!FBitSet(pDoor->pev->spawnflags, SF_DOOR_SILENT))// XDM
							if (!FStringNull(pDoor->pev->noiseMoving))
								STOP_SOUND(pDoor->edict(), DOOR_SNDCHAN_MOVING, STRINGV(pDoor->pev->noiseMoving));
...
// XDM3035c: special
bool CBaseDoor::HasTarget(string_t targetname)
{
	if (FStrEq(STRING(targetname), STRING(pev->netname)))
		return true;
...
// but spawn in the open position
	...
	{	// swap pos1 and pos2, put door at pos2, invert movement direction
		pev->angles = m_vecAngle2;// XDM3038c: FIX: WTF was this?!
		//Vector vecSav(m_vecAngle1);
		m_vecAngle2 = m_vecAngle1;
		m_vecAngle1 = pev->angles;//m_vecAngle1 = vecSav;
		pev->movedir *= -1;
...
void CMomentaryDoor::Use(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value)
{
	...
	//conprintf(1, "CMomentaryDoor::Use(%f)\n", value);
	...
	vec_t speed = delta.Length() * 10.0f;
	...
	if (speed != 0.0f)
	{
		...
		if (pev->nextthink < pev->ltime || pev->nextthink == 0)
			StartSound();//EMIT_SOUND(edict(), DOOR_SNDCHAN_MOVING, STRINGV(pev->noiseMoving), VOL_NORM, ATTN_NORM);
