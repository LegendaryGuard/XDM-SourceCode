...
Vector VecCheckToss(entvars_t *pev, const Vector &vecSpot1, Vector vecSpot2, float flGravityAdj)
{
	...
	float flGravity = g_psv_gravity?g_psv_gravity->value:DEFAULT_GRAVITY * flGravityAdj;
	...
	Vector vecMidPoint(vecSpot1 + (vecSpot2 - vecSpot1) * 0.5);// halfway point between Spot1 and Spot2
	TraceResult		tr;
	...
	vec_t distance1 = (vecMidPoint.z - vecSpot1.z);
	vec_t distance2 = (vecMidPoint.z - vecSpot2.z);
...
{
	Vector v(vecSrc); v -= vecEnemyPos;
	vec_t d = DotProduct(v, vecEnemyVel);
	vec_t a = DotProduct(vecEnemyVel, vecEnemyVel) - flProjectileSpeed * flProjectileSpeed;
	vec_t dsq = sqrtf(d * d - DotProduct(v, v) * a);
	float t1 = (d + dsq) / a;
	float t2 = (d - dsq) / a;
