...
// so instead, they record what changes should be made, and wait until this function gets called.
	...
	while (pEntity)
	{
		if (pEntity->IsAlias())
		{
			...
			if (pTempEntity)
			{
				...
				iTempOffset = OFFSET(pTempEntity->edict());
...
TYPEDESCRIPTION	CBaseAlias::m_SaveData[] = 
{
	...
	DEFINE_FIELD(CBaseAlias, m_iLFlags, FIELD_INTEGER),// TODO: should it be saved?
};
IMPLEMENT_SAVERESTORE(CBaseAlias, CPointEntity);

void CBaseAlias::KeyValue(KeyValueData *pkvd)
{
	if (FStrEq(pkvd->szKeyName, "lflags"))
	{
...
void CBaseAlias::ChangeValue(string_t iszValue)
{
...
void CInfoAlias::Use(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value)
{
	...
	{ //Old On/Off Code
		if (pev->spawnflags & SF_ALIAS_OFF)
		{
			...
			ClearBits(pev->spawnflags, SF_ALIAS_OFF);
		...
		else
		{
			...
			SetBits(pev->spawnflags, SF_ALIAS_OFF);
...
CBaseEntity *CMultiAlias::FollowAlias( CBaseEntity *pStartEntity )
{
	...
	if (m_iMode)
	{
		...
		iTempOffset = rand(); // throw away the first result - it's just the seed value // iTempOffset == just to STFU the compiler
...
void CTriggerChangeAlias::Use(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value)
{
	...
	if (pTarget && pTarget->IsAlias())
	{
		...
		if (pValue)
			((CBaseAlias *)pTarget)->ChangeValue(pValue->pev->targetname);
		else
...
void CInfoGroup::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if (pTarget && pTarget->IsAlias())
	{
		...
		((CBaseAlias*)pTarget)->ChangeValue(pev->targetname);
	}
	else if (pev->target)
	{
