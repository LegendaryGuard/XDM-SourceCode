...
{
	int ret = g_engfuncs.pfnPrecacheSound(s);
	if (ret == 0)
		(*g_engfuncs.pfnAlertMessage)(at_console, "PrecacheSound(%s) failed!\n", s);
	//else
	//	(*g_engfuncs.pfnAlertMessage)(at_console, "PrecacheSound(%s): %d\n", s, ret);
...
// SQB #define VEC_TO_ANGLES				(*g_engfuncs.pfnVecToAngles)
#define MOVE_TO_ORIGIN				(*g_engfuncs.pfnMoveToOrigin)
//#define oldCHANGE_YAW				(*g_engfuncs.pfnChangeYaw)
//#define CHANGE_PITCH				(*g_engfuncs.pfnChangePitch)
#define FIND_ENTITY_BY_STRING		(*g_engfuncs.pfnFindEntityByString)
#define GETENTITYILLUM				(*g_engfuncs.pfnGetEntityIllum)// Does not work for players
...
//#define MAKE_VECTORS				(*g_engfuncs.pfnMakeVectors)// Writes output vectors to globalvars
//#define ANGLE_VECTORS				(*g_engfuncs.pfnAngleVectors)// Better use custom functions from util_vector.h
//#define CREATE_ENTITY				(*g_engfuncs.pfnCreateEntity)
#if defined (_DEBUG)
inline void REMOVE_ENTITY(edict_t* e)
{
	//(*g_engfuncs.pfnAlertMessage)(at_aiconsole, "REMOVE_ENTITY()\n");
	(*g_engfuncs.pfnRemoveEntity)(e);
}
#else
#define REMOVE_ENTITY				(*g_engfuncs.pfnRemoveEntity)// DANGEROUS! Normally, should not be called! Example: if during RadiusDamage() some entity is erased, pStart/next search entity pointer is corrupted which leads to an infinite loop.
#endif
// Do not use #define CREATE_NAMED_ENTITY			(*g_engfuncs.pfnCreateNamedEntity)
#define MAKE_STATIC					(*g_engfuncs.pfnMakeStatic)// DANGEROUS! Destroys edict!
...
#define SERVER_EXECUTE				(*g_engfuncs.pfnServerExecute)// Call this after filling buffer with commands
...
// XDM3038b: this shit destroys 180 degree angles!! #define WRITE_ANGLE					(*g_engfuncs.pfnWriteAngle)
#define WRITE_COORD_NOCLAMP			(*g_engfuncs.pfnWriteCoord)
...
#define ALLOC_STRING				(*g_engfuncs.pfnAllocString)// WARNING: szValue string is unsafe after this!
...
#define ENTINDEX					(*g_engfuncs.pfnIndexOfEdict)// In HL entindexes are: 0: world, (1...NUMPLAYERS): players, (NUMPLAYERS...num_edicts): all other entities. NUMPLAYERS varies between 1 and MAX_PLAYERS
...
#define GET_MODEL_PTR				(*g_engfuncs.pfnGetModelPtr)// WARNING! Will crash with "Cache_UnlinkLRU: NULL link" if used on something other than studio models!
...
#define CMD_ARGC					(*g_engfuncs.pfnCmd_Argc)// 1 == command with no arguments
#define CMD_ARGV					(*g_engfuncs.pfnCmd_Argv)// (0) is the command itself
...
//#define SET_VIEW					(*g_engfuncs.pfnSetView) do not use directly
