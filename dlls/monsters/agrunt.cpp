...
//=========================================================
enum agrunt_animation_events_e
{
	AGRUNT_AE_HORNET1 = 1,
	AGRUNT_AE_HORNET2,
	AGRUNT_AE_HORNET3,
...
const char *CAGrunt::pDeathSounds[] =
{
	"agrunt/ag_die1.wav",
	//"agrunt/ag_die2.wav",
	//"agrunt/ag_die3.wav",
...
const char *CAGrunt::pShootSounds[] =
{
	"agrunt/ag_fire1.wav",
	"agrunt/ag_fire2.wav",
	"agrunt/ag_fire3.wav",
...
void CAGrunt::Spawn(void)
{
	...
	if (m_bloodColor == 0)// XDM3038a: no custom value specified
		m_bloodColor = BLOOD_COLOR_YELLOW;// XDM: not same as green
	if (m_iScoreAward == 0)
		m_iScoreAward = gSkillData.agruntScore;
	if (m_iGibCount == 0)
		m_iGibCount = 24;// XDM: big one
	if (m_flDamage1 == 0)// XDM3038c
		m_flDamage1 = gSkillData.monDmgHornet;
	if (m_flDamage2 == 0)// XDM3038c
		m_flDamage2 = gSkillData.agruntDmgPunch;
...
// TraceAttack
//=========================================================
void CAGrunt::TraceAttack(CBaseEntity *pAttacker, float flDamage, const Vector &vecDir, TraceResult *ptr, int bitsDamageType)
{
	if (ptr->iHitgroup == HITGROUP_ARMOR && FBitSet(bitsDamageType, DMGM_RICOCHET) && !FBitSet(bitsDamageType, DMG_IGNOREARMOR))// XDM3037
	{
		// hit armor
		if ( RANDOM_LONG( 0, 1 ) == 0 )
		{
			...
			MESSAGE_BEGIN( MSG_PVS, svc_temp_entity, ptr->vecEndPos );
	...
	CSquadMonster::TraceAttack(pAttacker, flDamage, vecDir, ptr, bitsDamageType);// XDM3038c AddMultiDamage( pAttacker, this, flDamage, bitsDamageType );
}
...
void CAGrunt :: PrescheduleThink (void)
{
	if ( ShouldSpeak() )
	{
		if ( m_flNextWordTime < gpGlobals->time )
		{
			...
			// play a new sound
//			IdleSound();
			EMIT_SOUND ( ENT(pev), CHAN_VOICE, pIdleSounds[ num ], GetSoundVolume(), ATTN_NORM );
...
void CAGrunt :: IdleSound (void)
{
// No, don't use this.
//	StopTalking();
//	EMIT_SOUND ( ENT(pev), CHAN_VOICE, pIdleSounds[RANDOM_LONG(0,ARRAYSIZE(pIdleSounds)-1)], GetSoundVolume(), ATTN_NORM );
}
	...
	case AGRUNT_AE_HORNET5:
		{
			...
			if (HasConditions( bits_COND_SEE_ENEMY))
			{
				...
				VectorAngles(vecDirToEnemy, angDir);//angDir = UTIL_VecToAngles( vecDirToEnemy );
				vecDirToEnemy.NormalizeSelf();
			}
			else
			{
				angDir = pev->angles;
			...
			// make angles +-180
			NormalizeAngle180(&angDir.x);// XDM3035c
#if defined (NOSQB)
			...
			{
			MESSAGE_BEGIN( MSG_PVS, svc_temp_entity, vecArmPos );
			...
			CHornet *pHornet = CHornet::CreateHornet(vecArmPos, vecAngles, vecDirToEnemy*300.0f, this, this, m_flDamage1, true);
			if (pHornet)
				pHornet->m_hEnemy = m_hEnemy;

			EMIT_SOUND_ARRAY_DYN2(CHAN_WEAPON, pShootSounds, ATTN_NORM, PITCH_NORM);// XDM3038c
		}
		break;
	case AGRUNT_AE_LEFT_FOOT:
		EMIT_SOUND_DYN(ENT(pev), CHAN_BODY, pStepSounds[RANDOM_INT2(1,3)], VOL_NORM, ATTN_NORM, 0, 75);
		break;
	case AGRUNT_AE_RIGHT_FOOT:
		EMIT_SOUND_DYN(ENT(pev), CHAN_BODY, pStepSounds[RANDOM_INT2(0,2)], VOL_NORM, ATTN_NORM, 0, 75);
		break;
	case AGRUNT_AE_PUNCH:// XDM3038c: TESTME?
	case AGRUNT_AE_BITE:// XDM3038c: TESTME?
	case AGRUNT_AE_LEFT_PUNCH:
		{
			CBaseEntity *pHurt = CheckTraceHullAttack( AGRUNT_MELEE_DIST, m_flDamage2, DMG_CLUB );
			if ( pHurt )
			{
				pHurt->pev->punchangle.y = -25.0f;
				pHurt->PunchPitchAxis(-8.0f);
				...
				if ( pHurt->IsPlayer() )
				{
					...
					pHurt->pev->velocity += gpGlobals->v_right * 250;
				}
				...
				UTIL_BloodDrips(vecArmPos, gpGlobals->v_forward, pHurt->BloodColor(), 25);// XDM
			}
			else
			{
...
void CAGrunt :: StartTask ( Task_t *pTask )
{
	...
	case TASK_AGRUNT_GET_PATH_TO_ENEMY_CORPSE:
		{
			...
			{
				//SPAM	ALERT ( at_aiconsole, "AGruntGetPathToEnemyCorpse failed!!\n" );
				TaskFail();
			}
		}
		break;
	...
	case TASK_AGRUNT_SETUP_HIDE_ATTACK:
		...
		if ( pEnemyMonsterPtr )
		{
			...
			// XDM3038a: UNDONE: FIXME: WTF was this?! UTIL_VecToAngles( m_vecEnemyLKP - pev->origin );
