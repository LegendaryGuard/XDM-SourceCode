...
public:
	...
	virtual int	Classify(void) { return CLASS_INSECT; }
...
void CLeech::Spawn(void)
{
	...
	if (m_bloodColor == 0)// XDM3038a: no custom value specified
		m_bloodColor = BLOOD_COLOR_YELLOW1;// XDM
	if (m_iScoreAward == 0)
		m_iScoreAward = gSkillData.leechScore;
...
void CLeech::SwitchLeechState(void)
{
	...
	else
	{
		...
		{
			m_hEnemy = pEnemy;
			SetMonsterState( MONSTERSTATE_COMBAT );
...
void CLeech::HandleAnimEvent(MonsterEvent_t *pEvent)
{
	...
	case LEECH_AE_ATTACK:
		...
		{
			Vector dir, face;
			AngleVectors( pev->angles, face, NULL, NULL );
			face.z = 0;
...
float CLeech::ObstacleDistance( CBaseEntity *pTarget )
{
	...
	UTIL_MakeAimVectors(pev->angles);// XDM3038a
...
void CLeech::SwimThink(void)
{
	...
	case MONSTERSTATE_COMBAT:
		...
		else
		{
			...
			targetYaw = VecToYaw(location);
			targetYaw = AngleDiff( targetYaw, UTIL_AngleMod( pev->angles.y ) );
...
void CLeech::Killed(CBaseEntity *pInflictor, CBaseEntity *pAttacker, int iGib)
{
	Vector			vecSplatDir;
	TraceResult		tr;
	...
	if (ShouldGibMonster(iGib))// XDM
	{
		UTIL_BloodDrips(pev->origin, UTIL_RandomBloodVector(), m_bloodColor, 40);
		Destroy();
		return;
	}
