...
void CFlockingFlyerFlock :: KeyValue(KeyValueData *pkvd)
{
	...
	else if (FStrEq(pkvd->szKeyName, "flFlockRadius"))
	{
		...
		CBaseMonster::KeyValue(pkvd);// XDM3038c: FIX
}
...
void CFlockingFlyerFlock :: SpawnFlock(void)
{
	...
	for ( iCount = 0 ; iCount < m_cFlockSize ; iCount++ )
	{
		...
		vecSpot += pev->origin;
		pBoid = (CFlockingFlyer *)Create("monster_flyer", vecSpot, pev->angles, g_vecZero, edict());// XDM3038: y not?
//		pBoid = GetClassPtr( (CFlockingFlyer *)NULL, "monster_flyer");// XDM
		if ( !pLeader )
		{
...
/*		pBoid->pev->velocity = g_vecZero;
	...
	pev->rendercolor.x	= 0;
	pev->rendercolor.y	= RANDOM_FLOAT(47, 255);
	pev->rendercolor.z	= RANDOM_FLOAT(47, 255);
}
...
// XDM3038?	pev->classname	= MAKE_STRING("monster_flyer");
...
//=========================================================
//=========================================================
/* XDM3038: this bullshit is causing too much crashes lately!
...
// XDM3038?	pev->classname	= MAKE_STRING("monster_flyer");
	...
	if (FStringNull(pev->model))// XDM3038
		ALERT(at_console, "ERROR: CFlockingFlyer has invalid model!\n");
	else
	{
		SET_MODEL(ENT(pev), STRING(pev->model));// XDM3037
		UTIL_SetSize(this, Vector(-5,-5,0), Vector(5,5,2));
...
void CFlockingFlyer :: MakeSound(void)
{
	if ( m_flAlertTime > gpGlobals->time )
	{
		...
		{
		case 0:	EMIT_SOUND(ENT(pev), CHAN_VOICE, "boid/boid_alert1.wav", VOL_NORM, ATTN_NORM); break;
		case 1:	EMIT_SOUND(ENT(pev), CHAN_VOICE, "boid/boid_alert2.wav", VOL_NORM, ATTN_NORM); break;
		}
	...
	switch ( RANDOM_LONG( 0, 1 ) )
	{
	case 0:	EMIT_SOUND(ENT(pev), CHAN_VOICE, "boid/boid_idle1.wav", VOL_NORM, ATTN_NORM); break;
	case 1:	EMIT_SOUND(ENT(pev), CHAN_VOICE, "boid/boid_idle2.wav", VOL_NORM, ATTN_NORM); break;
	}
}

//=========================================================
//=========================================================
void CFlockingFlyer::Killed(CBaseEntity *pInflictor, CBaseEntity *pAttacker, int iGib)
{
	CFlockingFlyer *pSquad = (CFlockingFlyer *)m_pSquadLeader;
	EMIT_SOUND_DYN(ENT(pev), CHAN_BODY, "boid/boid_alert1.wav", VOL_NORM, ATTN_NORM, 0, PITCH_NORM);
	pev->renderfx		= kRenderFxGlowShell;
	pev->rendercolor.x	= RANDOM_FLOAT(200, 255);
	pev->rendercolor.y	= RANDOM_FLOAT(200, 255);
	pev->rendercolor.z	= 0;
	while ( pSquad )
	{
...
void CFlockingFlyer :: IdleThink(void)
{
	...
	pev->rendercolor.y	= RANDOM_FLOAT(95, 255);
	pev->rendercolor.z	= RANDOM_FLOAT(95, 255);
}
...
void CFlockingFlyer :: SpreadFlock( )
{
	...
	while ( pList )
	{
		if ( pList != this && ( pev->origin - pList->pev->origin ).Length() <= AFLOCK_TOO_CLOSE )
		{
			...
			pList->pev->velocity.NormalizeSelf();
			pList->pev->velocity = (pList->pev->velocity + vecDir) * 0.5;
			pList->pev->velocity *= flSpeed;
		}
