...
public:
	...
	void MortarSpray(const Vector &direction, int spriteModel, int count);
...
public:
	...
	virtual BOOL CheckMeleeAttack1( float flDot, float flDist );	// Slash
	virtual BOOL CheckMeleeAttack2( float flDot, float flDist );	// Lay a crab
	virtual BOOL CheckRangeAttack1( float flDot, float flDist );	// Mortar launch
	virtual int	Save(CSave &save);
	virtual int	Restore(CRestore &restore);
...
//		CBaseEntity *pTarget = m_hTargetEnt;
//		if ( pTarget )
//			return pTarget->pev->sequence;// XDM3035c: nope
	...
	{
		if ( m_crabTime < gpGlobals->time && m_crabCount < gSkillData.bigmommaMaxChildren )
		{
			// Don't spawn crabs inside each other
			Vector mins = pev->origin - Vector( 32, 32, 0 );
			Vector maxs = pev->origin + Vector( 32, 32, 0 );
			CBaseEntity *pList[2];
			size_t count = UTIL_EntitiesInBox(pList, 2, mins, maxs, FL_MONSTER);
...
void CBigMomma :: HandleAnimEvent(MonsterEvent_t *pEvent)
{
	switch (pEvent->event)
	{
		...
		case BIG_AE_MELEE_ATTACK1:
		{
			...
			CBaseEntity *pList[8];
			size_t count = UTIL_EntitiesInBox( pList, 8, mins, maxs, FL_MONSTER|FL_CLIENT );
			CBaseEntity *pHurt = NULL;
			for (size_t i = 0; i < count && !pHurt; ++i)
			{
				if ( pList[i] != this )
				{
			...
			if ( pHurt )
			{
				...
				EMIT_SOUND_DYN(edict(), CHAN_WEAPON, RANDOM_SOUND_ARRAY(pAttackHitSounds), VOL_NORM, ATTN_NORM, 0, PITCH_NORM + RANDOM_LONG(-5,5));
			}
		}
		break;
...
void CBigMomma::TraceAttack(CBaseEntity *pAttacker, float flDamage, const Vector &vecDir, TraceResult *ptr, int bitsDamageType)
{
	if (ptr->iHitgroup != HITGROUP_HEAD && FBitSet(bitsDamageType, DMGM_RICOCHET) && !FBitSet(bitsDamageType, DMG_NOHITPOINT|DMG_IGNOREARMOR))// XDM3038c
	{
		// didn't hit the sack?
...
{
	CBaseEntity *pChild = Create(BIG_CHILDCLASS, pev->origin, pev->angles, g_vecZero, edict(), SF_MONSTER_FADECORPSE|SF_MONSTER_FALL_TO_GROUND | SF_NOREFERENCE|SF_NORESPAWN);// XDM3038b: explicitly fade
	if (pChild)
	{
		pChild->pev->owner = edict();// XDM3037: collision hack
		if (g_pGameRules->FAllowEffects())// XDM3037
		{
			...
			MESSAGE_BEGIN(MSG_PVS, svc_temp_entity, pev->origin);
				...
				WRITE_COORD(0);
				WRITE_COORD(-1);
				WRITE_SHORT(gSpitDebrisSprite);// model
				WRITE_BYTE(16);// count
				WRITE_BYTE(80);// speed
		...
		EMIT_SOUND_DYN( edict(), CHAN_WEAPON, RANDOM_SOUND_ARRAY(pBirthSounds), GetSoundVolume(), ATTN_NORM, 0, PITCH_NORM + RANDOM_LONG(-5,5) );
		m_crabCount++;
	}
}
...
void CBigMomma::LaunchMortar(void)
{
	...
	if (pBomb)
	{
		...
		EMIT_SOUND_DYN( edict(), CHAN_WEAPON, RANDOM_SOUND_ARRAY(pSackSounds), GetSoundVolume(), ATTN_NORM, 0, PITCH_NORM + RANDOM_LONG(-5,5) );
		if (g_pGameRules->FAllowEffects())// XDM3037
		{
			// spew the spittle temporary ents.
			MESSAGE_BEGIN(MSG_PVS, svc_temp_entity, startPos);
				WRITE_BYTE(TE_SPRITE_SPRAY);
				WRITE_COORD(startPos.x);// pos
				WRITE_COORD(startPos.y);
				WRITE_COORD(startPos.z);
				WRITE_COORD(0);// dir
				WRITE_COORD(0);
				WRITE_COORD(1);
				WRITE_SHORT(gSpitDebrisSprite);// model
...
void CBigMomma :: Spawn()
{
	...
	if (m_iGibCount == 0)
		m_iGibCount = 30;// XDM: big one
	if (m_iScoreAward == 0)
		m_iScoreAward = gSkillData.bigmommaScore;

	CBaseMonster::Spawn();// XDM3038b: Precache();
	//SET_MODEL(ENT(pev), STRING(pev->model));// XDM3037
	UTIL_SetSize(this, Vector( -32, -32, 0 ), Vector( 32, 32, 144 ) );// XDM
	pev->solid			= SOLID_SLIDEBOX;
	pev->movetype		= MOVETYPE_STEP;
	pev->view_ofs.Set(0, 0, 128);// position of the eyes relative to monster's origin.
...
//=========================================================
enum bigmomma_schedules_e
{
	SCHED_BIG_NODE = LAST_COMMON_SCHEDULE + 1,
	SCHED_NODE_FAIL,
};

enum bigmomma_tasks_e
{
...
void CBigMomma::StartTask( Task_t *pTask )
{
	...
	case TASK_MELEE_ATTACK1:
		...
		EMIT_SOUND_DYN( ENT(pev), CHAN_VOICE, RANDOM_SOUND_ARRAY(pAttackSounds), GetSoundVolume(), ATTN_NORM, 0, PITCH_NORM );
...
//	float distance = vecGrenadeVel.Length();
	...
	vecGrenadeVel = vecGrenadeVel * ( 0.5 / flytime );
...
void CBMortar::Animate(void)
{
	...
	if ( gpGlobals->time > pev->dmgtime )
	{
		...
		MortarSpray(-pev->velocity.Normalize(), gSpitSprite, 3 );
	}
	if ( pev->frame++ )
	{
...
{
	CBMortar *pSpit = GetClassPtr((CBMortar *)NULL, "bmortar");// XDM3038!
	if (pSpit == NULL)
		return NULL;
	...
	pSpit->pev->owner = pOwner;// still need this
	pSpit->m_hOwner.Set(pOwner);// XDM3037
	pSpit->pev->scale = 2.5;
	pSpit->SetThink(&CBMortar::Animate);
	pSpit->SetNextThink(0.1);

	if (g_pGameRules->FAllowEffects())
		pSpit->MortarSpray(Vector(0,0,1), gSpitSprite, 24);
...
{
	// splat sound
	int iPitch = RANDOM_FLOAT( 90, 100 );
	...
	{
	case 0: EMIT_SOUND_DYN( ENT(pev), CHAN_WEAPON, "bullchicken/bc_spithit1.wav", VOL_NORM, ATTN_NORM, 0, iPitch ); break;
	case 1: EMIT_SOUND_DYN( ENT(pev), CHAN_WEAPON, "bullchicken/bc_spithit2.wav", VOL_NORM, ATTN_NORM, 0, iPitch ); break;
	}
	...
	RadiusDamage(pev->origin, this, m_hOwner, gSkillData.bigmommaDmgBlast, gSkillData.bigmommaDmgBlast*2.0f, CLASS_NONE, DMG_ACID);// XDM3038a
	SetNextThink(0.01);
	SetTouchNull();
...
void CBMortar::MortarSpray(const Vector &direction, int spriteModel, int count)
{
	MESSAGE_BEGIN(MSG_PVS, svc_temp_entity, pev->origin);
		WRITE_BYTE(TE_SPRITE_SPRAY);
		WRITE_COORD(pev->origin.x);	// pos
