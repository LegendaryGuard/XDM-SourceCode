...
#include "game.h"
#include "gamerules.h"
#include "globals.h"// XDM3038

//=========================================================
// monster-specific schedule types
//=========================================================
enum assassin_schedules_e
{
...
//=========================================================
enum assassin_animation_events_e
{
	ASSASSIN_AE_SHOOT1 = 1,
	ASSASSIN_AE_TOSS1,
	ASSASSIN_AE_JUMP
};
...
public:
	...
	virtual BOOL CheckMeleeAttack1(float flDot, float flDist);	// jump
	// virtual BOOL CheckMeleeAttack2(float flDot, float flDist);
	virtual BOOL CheckRangeAttack1(float flDot, float flDist);	// shoot
	virtual BOOL CheckRangeAttack2(float flDot, float flDist);	// throw grenade
	virtual void StartTask (Task_t *pTask);
	virtual void RunAI(void);
	virtual void RunTask(Task_t *pTask);
	virtual void DeathSound(void);
	virtual void IdleSound(void);
	virtual void PainSound(void);// XDM
	virtual const char *GetDropItemName(void) { return "weapon_handgrenade";}// XDM3038
	...
	static const char *pDeathSounds[];// XDM
	static const char *pPainSounds[];// XDM
};
...
TYPEDESCRIPTION	CHAssassin::m_SaveData[] = 
{
	...
	DEFINE_FIELD( CHAssassin, m_flDeviation, FIELD_FLOAT ),
...
// XDM: sound arrays
const char *CHAssassin::pDeathSounds[] = 
{
	"hassassin/death1.wav",
	"hassassin/death2.wav",
	"hassassin/death3.wav",
	"hassassin/death4.wav",
};

const char *CHAssassin::pPainSounds[] = 
{
	"hassassin/pain1.wav",
	"hassassin/pain2.wav",
	"hassassin/pain3.wav",
...
{
	EMIT_SOUND_DYN( edict(), CHAN_VOICE, RANDOM_SOUND_ARRAY(pDeathSounds), GetSoundVolume(), ATTN_NORM, 0, RANDOM_LONG(95,105));
}
...
{
	EMIT_SOUND_DYN( edict(), CHAN_VOICE, RANDOM_SOUND_ARRAY(pPainSounds), GetSoundVolume(), ATTN_NORM, 0, RANDOM_LONG(95,105));
}
...
void CHAssassin :: Shoot (void)
{
	...
	if (g_pGameRules->FAllowEffects())
	{
		...
		EjectBrass(pev->origin + gpGlobals->v_up * 32 + gpGlobals->v_forward * 12, vecShellVelocity, pev->angles.y, m_iShell, SHELL_BRASS, TE_BOUNCE_SHELL); 
	}
	FireBullets(1, vecShootOrigin, vecShootDir, Vector(m_flDeviation, m_flDeviation, m_flDeviation), NULL, 2048, BULLET_9MM, 0, this, this/*m_hActivator*/, 0); // shoot +-8 degrees
...
void CHAssassin :: HandleAnimEvent(MonsterEvent_t *pEvent)
{
	...
	case ASSASSIN_AE_TOSS1:
		{
			UTIL_MakeVectors( pev->angles );
			Vector vecThrow = pev->origin + gpGlobals->v_forward * 34 + Vector(0, 0, 32);
			switch (RANDOM_LONG(0,2))// XDM: grenade types
			{
			case 0: CGrenade::ShootTimed(vecThrow, m_vecTossVelocity, 3.5, GREN_EXPLOSIVE , this, this); break;
			case 1: CGrenade::ShootTimed(vecThrow, m_vecTossVelocity, 3.5, GREN_FREEZE    , this, this); break;
			case 2: CGrenade::ShootTimed(vecThrow, m_vecTossVelocity, 3.5, GREN_TELEPORTER, this, this); break;
			}
	...
	case ASSASSIN_AE_JUMP:
		{
			...
			EMIT_SOUND_DYN( ENT(pev), CHAN_VOICE, "hassassin/jump.wav", GetSoundVolume(), ATTN_NORM, 0, RANDOM_LONG(95, 105));
...
void CHAssassin :: Spawn()
{
	...
	if (pev->armorvalue <= 0)
		pev->armorvalue = 10;// XDM
	if (m_bloodColor == 0)// XDM3038a: no custom value specified
		m_bloodColor = BLOOD_COLOR_RED;
	if (m_iScoreAward == 0)
		m_iScoreAward = gSkillData.hassassinScore;
	if (m_iGibCount == 0)
...
void CHAssassin :: Precache()
{
	...
	if (m_szSoundDir[0] == '\0')// XDM3038c
		strcpy(m_szSoundDir, "hassassin");

	CBaseMonster::Precache();// XDM3038

	PRECACHE_SOUND("weapons/pl_gun1.wav");
	PRECACHE_SOUND("weapons/pl_gun2.wav");
	PRECACHE_SOUND("hassassin/jump.wav");
//	PRECACHE_SOUND("hassassin/land.wav");// :-/
	PRECACHE_SOUND("hassassin/invis.wav");
	PRECACHE_SOUND_ARRAY( pDeathSounds );// XDM
...
BOOL CHAssassin :: CheckMeleeAttack1 ( float flDot, float flDist )
{
	...
	{
		TraceResult	tr;
		Vector vecDest = pev->origin + Vector( RANDOM_FLOAT( -64, 64), RANDOM_FLOAT( -64, 64 ), 160.0f );
		UTIL_TraceHull( pev->origin + Vector( 0.0f, 0.0f, 36.0f ), vecDest + Vector( 0.0f, 0.0f, 36.0f ), dont_ignore_monsters, human_hull, ENT(pev), &tr);
...
BOOL CHAssassin :: CheckRangeAttack2 ( float flDot, float flDist )
{
	...
	{
		Vector vecToss = VecCheckThrow(pev, GetGunPosition(), m_hEnemy->Center(), flDist, g_pWorld?g_pWorld->pev->gravity:1.0f); // use dist as speed to get there in 1 second // XDM3038: grenades now have normal gravity
		if (!vecToss.IsZero())
		{
...
void CHAssassin :: RunAI(void)
{
	...
	if (gSkillData.iSkillLevel != SKILL_HARD || m_hEnemy.Get() == NULL || pev->deadflag != DEAD_NO || m_Activity == ACT_RUN || m_Activity == ACT_WALK || !(pev->flags & FL_ONGROUND))
	...
	if (pev->renderamt > m_iTargetRanderamt)
	{
		...
		{
			EMIT_SOUND (ENT(pev), CHAN_BODY, "hassassin/invis.wav", GetSoundVolume(), ATTN_NORM );
			pev->renderfx = kRenderFxGlowShell;// XDM
			pev->rendercolor.Set(32,32,32);// XDM
		}
	...
	if (m_Activity == ACT_RUN || m_Activity == ACT_WALK)
	{
		...
		if (iStep)
		{
			...
			{
			case 0:	EMIT_SOUND( ENT(pev), CHAN_BODY, "common/npc_step1.wav", 0.5, ATTN_NORM);	break;// XDM3035a: use NPC sounds
			case 1:	EMIT_SOUND( ENT(pev), CHAN_BODY, "common/npc_step3.wav", 0.5, ATTN_NORM);	break;
			case 2:	EMIT_SOUND( ENT(pev), CHAN_BODY, "common/npc_step2.wav", 0.5, ATTN_NORM);	break;
			case 3:	EMIT_SOUND( ENT(pev), CHAN_BODY, "common/npc_step4.wav", 0.5, ATTN_NORM);	break;
			}
		}
	}
}
...
void CHAssassin :: RunTask ( Task_t *pTask )
{
	...
	case TASK_ASSASSIN_FALL_TO_GROUND:
		...
		{
//			EMIT_SOUND_DYN( ENT(pev), CHAN_VOICE, "hassassin/land.wav", GetSoundVolume(), ATTN_NORM, 0, RANDOM_LONG(95,105));// XDM
