...
//=========================================================
enum scientist_animation_events_e
{
	SCIENTIST_AE_HEAL = 1,
	SCIENTIST_AE_NEEDLEON,
	SCIENTIST_AE_NEEDLEOFF
};
...
public:
	...
	virtual const char *GetDropItemName(void);// XDM3038

	virtual float	CoverRadius(void) { return 1200; }		// Need more room for cover because scientists want to get far away!
	...
	static const char *pDeathSounds[];// XDM3038a
...
{
	if (FStrnEq(pkvd->szKeyName, "sound_pain", 10))// # at the end
	{
// FAIL! this overwrites sounds for ALL scientists globally		strcpy(m_pPainSounds[i], pkvd->szValue);
		// TODO: something like:
		//m_ppPainSounds = &pPainSounds;// default scientist sounds
		//m_ppPainSounds = &pCustomPainSounds;
		pkvd->fHandled = TRUE;
	}
	else
...
{
	// XDM3038a: WARNING! body affects GetDropItemName()! Set it BEFORE precaching!
	...
	if (m_iScoreAward == 0)
		m_iScoreAward = gSkillData.scientistScore;
...
void CScientist :: Precache(void)
{
	...
	if (m_szSoundDir[0] == '\0')// XDM3038c
		strcpy(m_szSoundDir, "scientist");
	if (FStringNull(m_iszSentencePrefix))// XDM3038a
		m_iszSentencePrefix = MAKE_STRING("SC");

	PRECACHE_SOUND_ARRAY(pPainSounds);
	PRECACHE_SOUND_ARRAY(pDeathSounds);

	// every new scientist must call this, otherwise when a level is loaded, nobody will talk (time is reset to 0)
	CTalkMonster::Precache();// XDM3038c TalkInit();
}
...
void CScientist :: TalkInit()
{
	...
	m_szGrp[TLK_SHOT] =			NULL;// XDM3038a: use simple pain sound
	m_szGrp[TLK_FEAR] =			DEFAULT_TLK_FEAR;
	m_szGrp[TLK_FEARPLAYER] =	DEFAULT_TLK_FEARPLAYER;
	m_szGrp[TLK_PLDEAD] =		"FEAR";
	// get voice for head
	switch (pev->body % 3)
	{
	default:
...
int	CScientist :: Classify (void)
{
	if (!HasMemory(bits_MEMORY_KILLED))// XDM3037: IsAlive() will return true right away, and we need to add score
		return m_iClass?m_iClass:CLASS_HUMAN_PASSIVE;// XDM
	else
...
int CScientist::TakeDamage(CBaseEntity *pInflictor, CBaseEntity *pAttacker, float flDamage, int bitsDamageType)
{
	...
	if ( pInflictor && pInflictor->IsPlayer())
	{
...
const char *CScientist::GetDropItemName(void)
{
	switch (pev->body % 3)
	{
	default:
	case HEAD_GLASSES:	return "item_healthkit"; break;
	case HEAD_EINSTEIN: return "item_antidote"; break;
	case HEAD_LUTHER:	return "item_sodacan";  break;
	case HEAD_SLICK:	return "item_battery";  break;
	}
}
...
void CScientist :: Scream(void)
{
	if ( FOkToSpeak() )
	{
		Talk( 10 );
		m_hTalkTarget = m_hEnemy;
		PlaySentence("SCREAM", RANDOM_FLOAT(3, 6), VOL_NORM, ATTN_NORM);
	}
}
...
//		if ( FOkToSpeak() )
		...
		PlaySentence("HEAL", 2, VOL_NORM, ATTN_IDLE);
		TaskComplete();
		break;
	...
	case TASK_SAY_FEAR:
		if ( FOkToSpeak() )
		{
			Talk( 2 );
			m_hTalkTarget = m_hEnemy;
			if (m_hEnemy && m_hEnemy->IsPlayer())// XDM3038c: NULL crash
				PlaySentence(m_szGrp[TLK_FEARPLAYER], 5, VOL_NORM, ATTN_NORM);
			else
...
void CScientist :: RunTask( Task_t *pTask )
{
	...
	case TASK_MOVE_TO_TARGET_RANGE_SCARED:
		{
			...
			{
				vec_t distance = ( m_vecMoveGoal - pev->origin ).Length2D();
	...
	case TASK_HEAL:
		...
		else if (m_hTargetEnt.Get())// XDM3038c
		{
			if (TargetDistance() > 90)
				TaskComplete();

			pev->ideal_yaw = VecToYaw(m_hTargetEnt->pev->origin - pev->origin);
	...
	case MONSTERSTATE_IDLE:
		...
		if ( HasConditions( bits_COND_HEAR_SOUND ) )
		{
			...
			if ( pSound )
			{
				...
				else if (pSound->m_iType & (bits_SOUND_WORLD | bits_SOUND_CARCASS | bits_SOUND_MEAT | bits_SOUND_GARBAGE | bits_SOUND_DEATH))// XDM3038a
				{
...
{
	if ( (m_healTime > gpGlobals->time) || (m_hTargetEnt.Get() == NULL) || (m_hTargetEnt->pev->health > (m_hTargetEnt->pev->max_health * 0.5)) )
		return FALSE;
...
public:
	...
	virtual void KeyValue(KeyValueData *pkvd);
	virtual const char *GetDropItemName(void) { return "item_healthkit";}// XDM3038
...
{
	if (pev->health <= 0)
		pev->health = gSkillData.scientistHealth * 0.6f;// XDM
...
void CSittingScientist :: Spawn( )
{
	...
	// in precache m_baseSequence = LookupSequence("sitlookleft");
...
{
	m_hTalkTarget = pSpeaker->MyMonsterPointer();
...
int CSittingScientist :: FIdleSpeak (void)
{
	...
	//ASSERT(UTIL_IsValidEntity(pFriend));
	...
	if (pFriend && RANDOM_LONG(0,1))
	{
		...
		SENTENCEG_PlayRndSz(edict(), m_szGrp[TLK_PQUESTION], GetSoundVolume(), ATTN_IDLE, 0, GetSoundPitch());
		// set global min delay for next conversation
		CTalkMonster::g_talkWaitTime = gpGlobals->time + RANDOM_FLOAT(4.8, 5.2);
		return TRUE;
	}
	...
	if (RANDOM_LONG(0,1))
	{
		SENTENCEG_PlayRndSz( edict(), m_szGrp[TLK_PIDLE], GetSoundVolume(), ATTN_IDLE, 0, GetSoundPitch());
		// set global min delay for next conversation
		CTalkMonster::g_talkWaitTime = gpGlobals->time + RANDOM_FLOAT(4.8, 5.2);
		return TRUE;
	}
