...
#define CONTROLLER_BALL_SPEED		800
...
public:
	...
	virtual BOOL CheckRangeAttack1(float flDot, float flDist);	// balls
	virtual BOOL CheckRangeAttack2(float flDot, float flDist);	// head
	virtual BOOL CheckMeleeAttack1(float flDot, float flDist);	// block, throw
...
void CController::Killed(CBaseEntity *pInflictor, CBaseEntity *pAttacker, int iGib)
{
	// shut off balls
	/*
	m_iBall[0] = 0;
	...
	*/
	...
	if (m_pBall[0])
	{
		m_pBall[0]->SetThink(&CBaseEntity::SUB_FadeOut);// XDM
		m_pBall[0]->SetNextThink(0.1);
//		m_pBall[0]->SUB_StartFadeOut();
		m_pBall[0] = NULL;
	}
	if (m_pBall[1])
	{
		m_pBall[1]->SetThink(&CBaseEntity::SUB_FadeOut);// XDM
		m_pBall[1]->SetNextThink(0.1);
//		m_pBall[1]->SUB_StartFadeOut();
		m_pBall[1] = NULL;
	}

	CSquadMonster::Killed(pInflictor, pAttacker, iGib);// new
...
bool CController::GibMonster(void)
{
	// delete balls
	if (m_pBall[0])
	{
	...
	//CGib::SpawnStickyGibs(this, pev->origin, 1);// XDM
	return CSquadMonster::GibMonster();
}
...
void CController :: HandleAnimEvent(MonsterEvent_t *pEvent)
{
	switch (pEvent->event)
	{
		...
		case CONTROLLER_AE_BALL_SHOOT:
		{
			...
			if (pBall)
			{
				pBall->pev->owner = edict();// XDM3037: old compatibility hack
				pBall->pev->velocity.Set(0, 0, 32);
...
void CController :: Spawn()
{
	...
	if (m_iScoreAward == 0)
		m_iScoreAward = gSkillData.controllerScore;
	if (m_iGibCount == 0)
		m_iGibCount = 14;// XDM: medium one
...
void CController :: Precache()
{
	...
	if (m_szSoundDir[0] == '\0')// XDM3038c
		strcpy(m_szSoundDir, "controller");

	CSquadMonster::Precache();// XDM3038a

	//PRECACHE_MODEL("models/stickygib.mdl");// XDM
	...
	case TASK_GET_PATH_TO_ENEMY_LKP:
		{
			...
			{
				// no way to get there =(
//SPAM				ALERT ( at_aiconsole, "GetPathToEnemyLKP failed!!\n" );
				TaskFail();
			}
			break;
		}
	...
	if (m_flShootEnd > gpGlobals->time)
	{
		...
		while (m_flShootTime < m_flShootEnd && m_flShootTime < gpGlobals->time)
		{
			...
			if (m_hEnemy != NULL)
			{
				...
				{
					m_vecEstVelocity *= 0.8;
				}
				vecDir = Intersect( vecSrc, m_hEnemy->BodyTarget( pev->origin ), m_vecEstVelocity, CONTROLLER_BALL_SPEED );
				float delta = 0.03490; // +-2 degree
				vecDir += Vector( RANDOM_FLOAT( -delta, delta ), RANDOM_FLOAT( -delta, delta ), RANDOM_FLOAT( -delta, delta ) ) * CONTROLLER_BALL_SPEED;
		...
		if ( CheckLocalMove ( pev->origin, pev->origin + vecDir * flCheckDist, pTargetEnt, &flDist ) != LOCALMOVE_VALID )
		{
			...
			else
			{
				...
				else
				{
					...
					{
						TaskFail();
						ALERT(at_aiconsole, "%s[%d] failed to move!\n", STRING(pev->classname), entindex());
...
class CControllerHeadBall : public CBaseMonster
{
	virtual void Spawn(void);
	virtual void Precache(void);
	virtual int ObjectCaps(void) { return FCAP_ACROSS_TRANSITION | FCAP_MUST_SPAWN; }// XDM
...
void CControllerHeadBall :: Spawn(void)
{
	...
	SET_MODEL(ENT(pev), "sprites/fireball.spr");// XDM
	...
	pev->renderamt = 255;
	pev->scale = 1.0;// XDM: 2.0 is too large for this sprite!
	pev->effects |= EF_DIMLIGHT;
	UTIL_SetSize(this, g_vecZero, g_vecZero);
	UTIL_SetOrigin(this, pev->origin);
	EMIT_SOUND(edict(), CHAN_BODY, "controller/headball_fly.wav", VOL_NORM, ATTN_IDLE);// XDM: looping sound
	SetThink(&CControllerHeadBall::HuntThink);
	SetTouch(&CControllerHeadBall::BounceTouch);
	m_vecIdeal = g_vecZero;
// XDM3037: set in Create()	m_hOwner = Instance(pev->owner);
	SetNextThink(0.1);
...
{
	PRECACHE_MODEL("sprites/fireball.spr");// XDM
	PRECACHE_SOUND("controller/headball_fly.wav");// XDM
...
void CControllerHeadBall :: HuntThink(void)
{
	...
	if (gpGlobals->time - pev->dmgtime > 5 || pev->renderamt < 64 || m_hEnemy.Get() == NULL || m_hOwner.Get() == NULL)// XDM || !IsInWorld())
	{
		STOP_SOUND(ENT(pev), CHAN_BODY, "controller/headball_fly.wav");// XDM
		Destroy();
		return;
	}
	...
	if ((m_hEnemy->Center() - pev->origin).Length() < 64)
	{
		...
		{
			ClearMultiDamage();
			pEntity->TraceAttack(m_hOwner, gSkillData.controllerDmgZap, pev->velocity.Normalize(), &tr, DMG_SHOCK);
...
{
	STOP_SOUND(ENT(pev), CHAN_BODY, "controller/headball_fly.wav");// XDM
	pev->velocity.SetLength(-1);// XDM3038c: nobody needs this now
	StreakSplash(pev->origin, pev->velocity, 6, 32, 80, 200);// XDM3038c
	UTIL_Remove( this );
}

void CControllerHeadBall :: MovetoTarget( const Vector &vecTarget )
{
	// accelerate
...
void CControllerHeadBall::BounceTouch(CBaseEntity *pOther)
{
	...
	if (g_pGameRules->FAllowEffects())
		StreakSplash(pev->origin, tr.vecPlaneNormal, 6, 16, 80, 160);// XDM3038c
}
...
void CControllerZapBall::ExplodeTouch(CBaseEntity *pOther)
{
	...
	{
		CBaseEntity *pOwner = NULL;// XDM3034
		if (m_hOwner != NULL)
			pOwner = m_hOwner;
		else
			pOwner = this;
		...
		ClearMultiDamage();
		pOther->TraceAttack(pOwner, gSkillData.controllerDmgBall, pev->velocity.Normalize(), &tr, DMG_ENERGYBEAM|DMG_SHOCK);
		//pOther->TraceAttack(pOwner, 1, pev->velocity.Normalize(), &tr, DMG_SHOCK );
		ApplyMultiDamage(this, pOwner);// XDM3034: 1st was pOwner too
		UTIL_EmitAmbientSound(ENT(pev), tr.vecEndPos, "weapons/electro4.wav", 0.3, ATTN_NORM, 0, RANDOM_LONG(90, 100));
	}
	StreakSplash(pev->origin, tr.vecPlaneNormal, 6, 32, 100, 200);// XDM3038c
