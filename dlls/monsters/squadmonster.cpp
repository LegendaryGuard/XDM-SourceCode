...
//	ASSERT( this->IsLeader() );
	...
	{
		for (short i = 0; i < MAX_SQUAD_MEMBERS-1;i++)
		{
...
bool CSquadMonster::NoFriendlyFire(bool playerAlly)
{
	...
	{
		Vector vecAngles;
		VectorAngles(m_hEnemy->Center() - pev->origin, vecAngles);
...
//-----------------------------------------------------------------------------
void CSquadMonster::ReportState(int printlevel)// XDM3038c
{
	CBaseMonster::ReportState(printlevel);
	if (!InSquad())
		conprintf(printlevel, "not ");

	conprintf(printlevel, "in squad, ");

	if (!IsLeader())
		conprintf(printlevel, "not ");

	conprintf(printlevel, "leader.\n");
}
