...
void CBaseMonster :: ChangeSchedule ( Schedule_t *pNewSchedule )
{
	...
	if (m_pSchedule)
	{
		if ((m_pSchedule->iInterruptMask & bits_COND_HEAR_SOUND) && m_pSchedule->iSoundMask == 0)
		{
			ALERT(at_aiconsole, "Schedule \"%s\": COND_HEAR_SOUND with no sound mask!\n", m_pSchedule->pName);
		}
		else if (m_pSchedule->iSoundMask && !(m_pSchedule->iInterruptMask & bits_COND_HEAR_SOUND))
		{
			ALERT(at_aiconsole, "Schedule \"%s\": Sound mask without COND_HEAR_SOUND!\n", m_pSchedule->pName);
		}

		if ( !ScheduleFromName( m_pSchedule->pName ) )
		{
		...
		if ( !FScheduleValid() || m_MonsterState != m_IdealMonsterState )
		{
				 ...
				 (m_IdealMonsterState != MONSTERSTATE_SCRIPT || m_IdealMonsterState == m_MonsterState) )
			{
				if (	(m_afConditions && !HasConditions(bits_COND_SCHEDULE_DONE)) ||
						...
						((m_MonsterState == MONSTERSTATE_COMBAT) && (m_hEnemy.Get() == NULL))	)
				{
					GetIdealState();
				}
			}
	...
	case TASK_PLAY_SEQUENCE_FACE_TARGET:
		{
			...
			if ( pTarget )
			{
				pev->ideal_yaw = VecToYaw(pTarget->pev->origin - pev->origin);
				ChangeYaw( pev->yaw_speed );
			}
	...
	case TASK_DIE:
		{
			if ( m_fSequenceFinished && pev->frame >= 255 )
			{
				...
				SetThink(&CBaseMonster::SUB_DoNothing);// XDM3038c: don't SetThinkNull();
				SetNextThink(1.0);// we need CheckEnvironment() to work on dead bodies
				StopAnimation();
				...
				if ( !BBoxFlat() )
				{
					...
					UTIL_SetSize(this, pev->mins, Vector(pev->maxs.x, pev->maxs.y, pev->mins.z + 1));
					//UTIL_SetSize ( pev, Vector ( pev->mins.x, pev->mins.y, pev->mins.z ), Vector ( pev->maxs.x, pev->maxs.y, pev->mins.z + 1 ) );

				// XDM3035: it was SO hard to figure out where to place this code!!
				RunPostDeath();// XDM3037
			}
			break;
		}
...
void CBaseMonster :: StartTask ( Task_t *pTask )
{
	...
	case TASK_FIND_NEAR_NODE_COVER_FROM_ENEMY:
		{
			...
			if ( FindCover( m_hEnemy->pev->origin, m_hEnemy->pev->view_ofs, 0, pTask->flData ) )
				TaskComplete();// try for cover farther than the FLData from the schedule.
			else
				TaskFail();// no coverwhatsoever.
	...
	case TASK_WALK_TO_TARGET:
		{
			...
			else
			{
				...
				{
					if (m_hTargetEnt.Get() == NULL || !MoveToTarget(newActivity, 2))
					{
						TaskFail();
						ALERT(at_aiconsole, "%s[%d] Failed to reach target!!!\n", STRING(pev->classname), entindex());
						RouteClear();
					}
				}
			}
	...
	case TASK_GET_PATH_TO_ENEMY_CORPSE:
		{
			Vector fw;
			AngleVectors(pev->angles, fw, NULL, NULL);// XDM3035a
	...
	case TASK_SOUND_IDLE:
		{
			if (!FBitSet(pev->spawnflags, SF_MONSTER_GAG) || HasConditions(bits_COND_HEAR_SOUND|bits_COND_SEE_CLIENT) || !FNullEnt(FIND_CLIENT_IN_PVS(edict())))//HasMemory(bits_MEMORY_SEEN_PLAYER))// XDM3038c: experimental
				IdleSound();
	...
	default:
		{
			ALERT(at_aiconsole, "%s[%d]::StartTask(%d) unknown task!\n", STRING(pev->classname), entindex(), pTask->iTask);
	...
	if (HasConditions(bits_COND_TASK_FAILED))
		ALERT(at_aiconsole, "%s[%d]::StartTask(%d) task failed!\n", STRING(pev->classname), entindex(), pTask->iTask);
#endif
}
...
Task_t	*CBaseMonster :: GetTask (void)
{
	if ( m_iScheduleIndex < 0 || m_iScheduleIndex >= m_pSchedule->cTasks )
		return NULL;// m_iScheduleIndex is not within valid range for the monster's current schedule.
	else
	...
	case MONSTERSTATE_COMBAT:
		{
			...
			else
			{
				...
				if (HasConditions(bits_COND_CAN_MELEE_ATTACK1))// XDM3038c: make melee attacks higher priority so monsters won't look as stupid
				{
				...
				else if ( !FacingIdeal() )
				{
					...
					conprintf(2, "%s[%d]: No suitable combat schedule!\n", STRING(pev->classname), entindex());
			}
			break;
		}
	...
	case MONSTERSTATE_SCRIPT:
		{
			...
			if ( !m_pCine )
			{
				ALERT( at_aiconsole, "Script failed for %s[%d]\n", STRING(pev->classname), entindex());
				CineCleanup();
				return GetScheduleOfType( SCHED_IDLE_STAND );
			}
