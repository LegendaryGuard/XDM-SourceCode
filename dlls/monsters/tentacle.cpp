...
public:
	...
	virtual MONSTERSTATE GetIdealState(void) { return MONSTERSTATE_IDLE; }
	virtual int CanPlaySequence(BOOL fDisregardState, int interruptLevel) { return TRUE; }
	virtual void AlertSound(void);// XDM3038c
...
//	static int g_fSquirmSound;
	...
	static const char *pAlertSounds[];// XDM3038c
...
const char *CTentacle::pAlertSounds[] =
{
...
typedef enum tentacle_animations_e
{
...
// XDM3035c: developers assume there's only one spot with tentacles on the map, so they made these global
int g_fFlySound = FALSE;
int g_fSquirmSound = FALSE;
...
void CTentacle :: Spawn( )
{
	...
	if (m_iScoreAward == 0)
		m_iScoreAward = gSkillData.tentacleScore;
	if (m_iGibCount == 0)
		m_iGibCount = 100;
	...
	pev->effects		= 0;
	//pev->health			= 75;
	pev->max_health		= pev->health;// XDM3035c
	pev->sequence		= 0;
	...
	pev->flags			|= FL_MONSTER | FL_GODMODE;// XDM3035c: so bots will avoid rather than attack this monster
...
void CTentacle :: Precache( )
{
	...
	//PRECACHE_SOUND("tentacle/te_search1.wav"); unused
...
void CTentacle::AlertSound(void)
{
...
void CTentacle :: Cycle(void)
{
	...
	case TENTACLE_ANIM_Lev3_Rear_Idle:
		if (dy < 0 && dy > -m_flMaxYaw)
			dy = -m_flMaxYaw;
		else if (dy > 0 && dy < m_flMaxYaw)
			dy = m_flMaxYaw;
		break;
	default:
		if (dy < -m_flMaxYaw)
			dy = -m_flMaxYaw;
		else if (dy > m_flMaxYaw)
			dy = m_flMaxYaw;
	}
	...
	if (m_fSequenceFinished)
	{
		...
		if (pev->health <= 1)
		{
			...
			if (pev->sequence == TENTACLE_ANIM_Pit_Idle)
			{
				...
				SetNextThink(gSkillData.tentacleHideTime);
				if (IsMultiplayer())
					pev->framerate = 0.0f;
//					pev->effects |= EF_NODRAW;// XDM3035c: bots will not detect any danger
//				ALERT(at_console, "TENTACLE_ANIM_Pit_Idle\n");
			}
		}
		...
		case TENTACLE_ANIM_Lev3_Tap:
			{
				...
				vecSrc = pev->origin + Vector( 0.0f, 0.0f, MyHeight() - 4);
				UTIL_TraceLine( vecSrc, vecSrc + gpGlobals->v_forward * 512, ignore_monsters, ENT( pev ), &tr1 );

				vecSrc = pev->origin + Vector( 0.0f, 0.0f, MyHeight() + 8);
...
/*BAD!	if (pev->sequence == TENTACLE_ANIM_Pit_Idle)// all the time during this animation
	{
		if (IsMultiplayer())
			pev->framerate = 0.0f;// pretend to be inactive
//	not here				pev->effects &= ~EF_NODRAW;// XDM3035c: bots will not detect any danger
	}
	else */if (m_flPrevSoundTime + 2.0 > gpGlobals->time)
	{
...
void CTentacle :: HandleAnimEvent(MonsterEvent_t *pEvent)
{
	...
	case 7: // roar
		...
		UTIL_EmitAmbientSound(ENT(pev), pev->origin + Vector( 0.0f, 0.0f, MyHeight()), sound, 1.0, ATTN_NORM, 0, 100);
		break;

	/* unused case 8: // search
		switch (RANDOM_LONG(0,1))
		{
...
void CTentacle::Killed(CBaseEntity *pInflictor, CBaseEntity *pAttacker, int iGib)
{
	m_iGoalAnim = TENTACLE_ANIM_Pit_Idle;
	return;
}
