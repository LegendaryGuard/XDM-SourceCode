...
#define	HGRUNT_NUM_GRENADE_TYPES		3

#define HITGROUP_ARMOR_HGRUNT			11
...
// XDM3035b: no more radioactive grenades
int iHGruntGrenType[HGRUNT_NUM_HEADS][HGRUNT_NUM_GRENADE_TYPES] =
{//	CGrenade types: 1st					2nd				3rd
/*GRUNT*/			{GREN_EXPLOSIVE,	GREN_POISON,	GREN_EXPLOSIVE},
/*COMMANDER*/		{GREN_EXPLOSIVE,	GREN_FREEZE,	GREN_NAPALM},
/*SHOTGUN*/			{GREN_EXPLOSIVE,	GREN_POISON,	GREN_NAPALM},
/*M203*/			{GREN_EXPLOSIVE,	GREN_FREEZE,	GREN_EXPLOSIVE}
};

//=========================================================
// Monster's Anim Events Go Here
//=========================================================
enum hgrunt_animation_events_e
{
	HGRUNT_AE_RELOAD = 2,
	HGRUNT_AE_KICK,
	HGRUNT_AE_BURST1,
	...
	HGRUNT_AE_GREN_DROP,
	HGRUNT_AE_CAUGHT_ENEMY,// grunt established sight with an enemy (player only) that had previously eluded the squad.
	HGRUNT_AE_DROP_GUN,// grunt (probably dead) is dropping his mp5.
	HGRUNT_AE_STEP_L,// XDM: step sounds
	HGRUNT_AE_STEP_R
};
...
public:
	...
	int		m_cClipSize;
	int		m_iLastUsedGrenadeType;

	//int m_voicePitch; XDM: moved to CBaseMonster
...
const char *CHGrunt::pStepSounds[] = 
{
	"hgrunt/gr_step1.wav",
	"hgrunt/gr_step2.wav",
	"hgrunt/gr_step3.wav",
...
void CHGrunt::Spawn()
{
	...
	if (pev->armorvalue <= 0)
		pev->armorvalue = gSkillData.batteryCapacity * (gSkillData.iSkillLevel+1);// XDM3035a
	if (m_bloodColor == 0)// XDM3038a: no custom value specified
		m_bloodColor = BLOOD_COLOR_RED;
	if (m_iScoreAward == 0)
		m_iScoreAward = gSkillData.hgruntScore;
	if (m_iGibCount == 0)
		m_iGibCount = GIB_COUNT_HUMAN;// XDM: medium one
	if (m_flDamage2 == 0)// XDM3038c
		m_flDamage2 = gSkillData.hgruntDmgKick;
	...
	pev->body			= 0;// XDM3038b: some mappers leave me no choice! (magnum.bsp)
	...
	m_iLastUsedGrenadeType = 0;// XDM3035b
	m_afCapability		= bits_CAP_SQUAD | bits_CAP_TURN_HEAD | bits_CAP_DOORS_GROUP;
	m_fEnemyEluded		= FALSE;
	m_fFirstEncounter	= TRUE;// this is true when the grunt spawns, because he hasn't encountered an enemy yet.
	m_HackedGunPos.Set(0, 0, 55);
	...
	{
		SetBodygroup(HG_GUN_GROUP, GUN_SHOTGUN);
		m_cClipSize		= SHOTGUN_MAX_CLIP;// XDM: like player clip
	}
	else
	{
		SetBodygroup(HG_GUN_GROUP, GUN_MP5);// XDM3035b: may respawn with no gun!
		m_cClipSize = MP5_MAX_CLIP;//GRUNT_CLIP_SIZE;
	}
	m_cAmmoLoaded = m_cClipSize;

	if (FBitSet(pev->weapons, HGRUNT_GRENADELAUNCHER))
	{
		SetBodygroup(HG_HEAD_GROUP, HEAD_M203);
	...
	else
	{
		...
		if (FBitSet(pev->weapons, HGRUNT_SHOTGUN))
			SetBodygroup(HG_HEAD_GROUP, HEAD_SHOTGUN);
	}
...
void CHGrunt::Precache(void)
{
	...
	if (pev->deadflag != DEAD_DEAD)// XDM3038c: used by deadgrunts
	{
	...
	// get voice pitch
	if (m_voicePitch <= 0)// XDM3038a: mappers
	{
		if (RANDOM_LONG(0,1))
	...
	m_iShotgunShell = m_iBrassShell = PRECACHE_MODEL("models/shell.mdl");// XDM3035a
	//m_iShotgunShell = PRECACHE_MODEL("models/shotgunshell.mdl");

	// XDM3038
	if (FBitSet(pev->weapons, HGRUNT_SHOTGUN))
		UTIL_PrecacheWeapon("weapon_shotgun");// "weapons/shotgun_pump.wav"
	else
		UTIL_PrecacheWeapon("weapon_9mmAR");// weapons/mp5_reload.wav, weapons/hks*.wav
...
void CHGrunt::SpeakSentence(void)
{
	...
	if (FOkToSpeak())
	{
		SENTENCEG_PlayRndSz(edict(), pGruntSentences[m_iSentence], HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
		JustSpoke();
	}
}
	...
	if (pTarget->IsProjectile())
	{
		if (pTarget->IsMonster())
		{
			if (targetclass == CLASS_PLAYER_BIOWEAPON)
				...
				return R_FR;// get away from projectiles
		}
		return R_FR;
	}
	else if (pTarget->IsMonster())// XDM3035b: fasterize a bit
	{
		//if (pTarget->Classify() == CLASS_ALIEN_PASSIVE)
		//	return R_NO;
...
//=========================================================
bool CHGrunt::GibMonster (void)
{
	DropItems();// XDM
	return CBaseMonster::GibMonster();
}
...
BOOL CHGrunt::FOkToSpeak(void)
{
	...
	if (IsMultiplayer() && FNullEnt(FIND_CLIENT_IN_PVS(edict())))// XDM3035a
		return FALSE;
...
BOOL CHGrunt::CheckMeleeAttack1 ( float flDot, float flDist )
{
	...
	if (pEnemy && flDist <= 64 && flDot >= 0.7)
	{
		if (pEnemy->pev->health < pev->max_health)// XDM3038c: don't bother attacking someone stronger
		{
			int iClass = pEnemy->Classify();
			if (iClass != CLASS_ALIEN_BIOWEAPON && iClass != CLASS_PLAYER_BIOWEAPON)
				return TRUE;
		}
	}
	return FALSE;
...
{
	if (!HasConditions(bits_COND_ENEMY_OCCLUDED) && flDist <= BULLET_DEFAULT_DIST && flDot >= 0.5 && m_hEnemy.Get() && NoFriendlyFire())// XDM3038c
	{
		if (!m_hEnemy->IsPlayer() && flDist <= 64)
			return FALSE;// kick nonclients, but don't shoot at them.
...
BOOL CHGrunt::CheckRangeAttack2 ( float flDot, float flDist )
{
	...
	if (InSquad())
	{
		if (SquadMemberInRange(vecTarget, gSkillData.DmgHandGrenadeExplosive * GREN_DAMAGE_TO_RADIUS))
		{
	...
	if ((vecTarget - pev->origin).Length2D() <= gSkillData.DmgHandGrenadeExplosive * GREN_DAMAGE_TO_RADIUS)// XDM3038c: radius
	{
	...
	if (FBitSet(pev->weapons, HGRUNT_HANDGRENADE))
	{
		Vector vecToss = VecCheckToss(pev, GetGunPosition(), vecTarget, g_pWorld?g_pWorld->pev->gravity:1.0f);// XDM3038: grenades now have normal gravity
		if (!vecToss.IsZero())
		{
	...
	{
		Vector vecToss(VecCheckThrow(pev, GetGunPosition(), vecTarget, gSkillData.hgruntGrenadeSpeed, g_pWorld?g_pWorld->pev->gravity:1.0f));// XDM3038: grenades now have normal gravity
		if (!vecToss.IsZero())
		{
			m_vecTossVelocity = vecToss;
			// throw a hand grenade
...
//=========================================================
void CHGrunt::TraceAttack(CBaseEntity *pAttacker, float flDamage, const Vector &vecDir, TraceResult *ptr, int bitsDamageType)
{
	// check for helmet shot
	if (ptr->iHitgroup == HITGROUP_ARMOR || ptr->iHitgroup == HITGROUP_ARMOR_HGRUNT)// Warning: HLBUG: HITGROUP_ARMOR differs in this model
	{
		// make sure we're wearing one
		if (GetBodygroup(HG_HEAD_GROUP) == HEAD_GRUNT && FBitSet(bitsDamageType, DMGM_RICOCHET) && !FBitSet(bitsDamageType, DMG_IGNOREARMOR))
		{
			// absorb damage
			flDamage -= 20;
			if (flDamage <= 0)
		...
		// XDM3038c: WRONG! ptr->iHitgroup = HITGROUP_HEAD;
	}
...
int CHGrunt::TakeDamage(CBaseEntity *pInflictor, CBaseEntity *pAttacker, float flDamage, int bitsDamageType)
{
	...
	if (pInflictor && m_hEnemy.Get() == NULL)// XDM3035: XDM3038a: pInflictor can be NULL
	{
		if (pInflictor->IsMonster() && /*recursion! Use CBaseMonster version! */CBaseMonster::IRelationship(pInflictor) >= R_DL)
			m_hEnemy = pInflictor;
	}

	if (GetBodygroup(HG_HEAD_GROUP) == HEAD_GRUNT)// XDM3035a
	{
		if (!FBitSet(bitsDamageType, DMG_IGNOREARMOR))
		{
			if (FBitSet(bitsDamageType, DMG_SLOWBURN))// less protection
				flDamage *= 0.75f;
			else if (FBitSet(bitsDamageType, DMG_NERVEGAS | DMG_POISON | DMG_ACID))// more protection
				flDamage *= 0.1f;
		}
	}
	else if (GetBodygroup(HG_HEAD_GROUP) == HEAD_SHOTGUN)
	{
		if (!FBitSet(bitsDamageType, DMG_IGNOREARMOR))
		{
			if (FBitSet(bitsDamageType, DMG_SLOWBURN))// less protection
				flDamage *= 0.75f;
			else if (FBitSet(bitsDamageType, DMG_ACID))// more protection
				flDamage *= 0.4f;
		}
	}
...
void CHGrunt::IdleSound(void)
{
	if (FOkToSpeak() && (g_fGruntQuestion || RANDOM_LONG(0,1)))
	{
		if (!g_fGruntQuestion)
		{
			...
			{
			case 0: // check in
				SENTENCEG_PlayRndSz(edict(), "HG_CHECK", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
				g_fGruntQuestion = 1;
				break;
			case 1: // question
				SENTENCEG_PlayRndSz(edict(), "HG_QUEST", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
				g_fGruntQuestion = 2;
				break;
			case 2: // statement
				SENTENCEG_PlayRndSz(edict(), "HG_IDLE", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
				break;
			}
		}
		else
		{
			...
			{
			case 1: // check in
				SENTENCEG_PlayRndSz(edict(), "HG_CLEAR", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
				break;
			case 2: // question
				SENTENCEG_PlayRndSz(edict(), "HG_ANSWER", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
				break;
			}
			g_fGruntQuestion = 0;
		}
...
//=========================================================
Vector CHGrunt::GetGunPosition(void)
{
	if (m_fStanding)
...
void CHGrunt::Shoot(void)
{
	...
	FireBullets(1, vecShootOrigin, vecShootDir, VECTOR_CONE_6DEGREES, NULL, 3072, BULLET_MP5, 0, this, this/*m_hActivator*/, 0);

	if (g_pGameRules->FAllowEffects())
	{
		Vector vecShellVelocity = gpGlobals->v_right * RANDOM_FLOAT(40,90) + gpGlobals->v_up * RANDOM_FLOAT(75,200) + gpGlobals->v_forward * RANDOM_FLOAT(-40, 40);
		EjectBrass(vecShootOrigin - vecShootDir * 24, vecShellVelocity, pev->angles.y, m_iBrassShell, SHELL_BRASS, TE_BOUNCE_SHELL);
	}
	pev->effects |= EF_MUZZLEFLASH;
	switch (RANDOM_LONG(0,2))
	{
		case 0: EMIT_SOUND(edict(), CHAN_WEAPON, "weapons/hks1.wav", VOL_NORM, ATTN_NORM); break;
		case 1: EMIT_SOUND(edict(), CHAN_WEAPON, "weapons/hks2.wav", VOL_NORM, ATTN_NORM); break;
		case 2: EMIT_SOUND(edict(), CHAN_WEAPON, "weapons/hks3.wav", VOL_NORM, ATTN_NORM); break;
	}
...
void CHGrunt::Shotgun(void)
{
	...
	FireBullets(SHOTGUN_USE_AMMO1*SHOTGUN_PELLETS_PER_AMMO, vecShootOrigin, vecShootDir, VECTOR_CONE_15DEGREES, NULL, 2048, BULLET_BUCKSHOT, 0, this, this/*m_hActivator*/, 0); // shoot +-7.5 degrees

	if (g_pGameRules->FAllowEffects())
	{
		Vector vecShellVelocity = gpGlobals->v_right * RANDOM_FLOAT(40,90) + gpGlobals->v_up * RANDOM_FLOAT(75,200) + gpGlobals->v_forward * RANDOM_FLOAT(-40, 40);
		EjectBrass(vecShootOrigin - vecShootDir * 24, vecShellVelocity, pev->angles.y, m_iShotgunShell, SHELL_BUCKSHOT, TE_BOUNCE_SHOTSHELL);
	}
	pev->effects |= EF_MUZZLEFLASH;
	EMIT_SOUND(edict(), CHAN_WEAPON, "weapons/shotgun_fire1.wav", VOL_NORM, ATTN_NORM);
	m_cAmmoLoaded -= SHOTGUN_USE_AMMO1;// take away a bullet!
...
void CHGrunt::HandleAnimEvent(MonsterEvent_t *pEvent)
{
	...
	switch (pEvent->event)
	{
		...
		{
			if (FBitSet(pev->weapons, HGRUNT_SHOTGUN))// XDM: these sound are precached in weapons
				EMIT_SOUND(edict(), CHAN_WEAPON, "weapons/shotgun_pump.wav", 0.8, ATTN_NORM);
			else
				EMIT_SOUND(edict(), CHAN_WEAPON, "weapons/mp5_reload.wav", 0.8, ATTN_NORM);
		...
		case HGRUNT_AE_GREN_TOSS:
		{
			...
			m_iLastUsedGrenadeType = iHGruntGrenType[GetBodygroup(HG_HEAD_GROUP)][RANDOM_LONG(0,HGRUNT_NUM_GRENADE_TYPES-1)];
			CGrenade::ShootTimed(GetGunPosition(), m_vecTossVelocity, 3.5, m_iLastUsedGrenadeType, this, this);
			// XDM: human grunts can't throw hi-tech teleporter grenades!
			m_fThrowGrenade = FALSE;
		...
		{
			EMIT_SOUND(edict(), CHAN_WEAPON, "weapons/glauncher.wav", 0.8, ATTN_NORM);
		...
		case HGRUNT_AE_GREN_DROP:
		{
			UTIL_MakeVectors( pev->angles );
			m_iLastUsedGrenadeType = iHGruntGrenType[GetBodygroup(HG_HEAD_GROUP)][RANDOM_LONG(0,HGRUNT_NUM_GRENADE_TYPES-1)];
			CGrenade::ShootTimed(pev->origin + gpGlobals->v_forward * 17 - gpGlobals->v_right * 27 + gpGlobals->v_up * 6, g_vecZero, 3, m_iLastUsedGrenadeType, this, this);// XDM
		}
		break;
		...
		case HGRUNT_AE_KICK:
		{
			...
			if ( pHurt )
			{
				...
				pHurt->TakeDamage(this, this, m_flDamage2, DMG_CLUB);// TODO: TraceAttack()?
				EMIT_SOUND(ENT(pHurt->pev), CHAN_BODY, "common/bodyhit1.wav", VOL_NORM, ATTN_NORM);// XDM3037
				if (pHurt->IsPushable())// XDM3037: do damage to everyone, push only pushables
				{
					UTIL_MakeVectors( pev->angles );
					pHurt->pev->velocity += (gpGlobals->v_forward*2 + gpGlobals->v_up)*DamageForce(m_flDamage2);// XDM3037 = pHurt->pev->velocity + gpGlobals->v_forward * 100 + gpGlobals->v_up * 50;
				}
			}
		}
		break;
		...
		case HGRUNT_AE_CAUGHT_ENEMY:
		{
			if ( FOkToSpeak() )
			{
				SENTENCEG_PlayRndSz(edict(), "HG_ALERT", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
				 JustSpoke();
			}
		...
		case HGRUNT_AE_STEP_L:
		{
			if (RANDOM_LONG(0,1) == 0)
				EMIT_SOUND(edict(), CHAN_BODY, pStepSounds[0], VOL_NORM, ATTN_NORM);
			else
				EMIT_SOUND(edict(), CHAN_BODY, pStepSounds[1], VOL_NORM, ATTN_NORM);
		}
		break;

		case HGRUNT_AE_STEP_R:
		{
			if (RANDOM_LONG(0,1) == 0)
				EMIT_SOUND(edict(), CHAN_BODY, pStepSounds[2], VOL_NORM, ATTN_NORM);
			else
				EMIT_SOUND(edict(), CHAN_BODY, pStepSounds[3], VOL_NORM, ATTN_NORM);
		}
		break;
...
/*#if 0
		if ( RANDOM_LONG(0,99) < 5 )
		{
			...
			if (FOkToSpeak())
			{
				SENTENCEG_PlayRndSz(edict(), "HG_PAIN", HGRUNT_SENTENCE_VOLUME, ATTN_NORM, 0, PITCH_NORM);
				JustSpoke();
				return;
			}
		}
...
Task_t	tlGruntCombatFace1[] =
{
	...
	{ TASK_SET_ACTIVITY,			(float)ACT_IDLE				},
	{ TASK_FACE_ENEMY,				(float)0					},
	{ TASK_WAIT,					(float)0.5					},// XDM: was 1.5
...
//=========================================================
// XDM: run to the sound's position
//=========================================================
Task_t	tlGruntCheckSound[] =
{
	{ TASK_STOP_MOVING,				(float)0				},
...
Schedule_t	slGruntCheckSound[] =
{
	{
		tlGruntCheckSound,
		ARRAYSIZE ( tlGruntCheckSound ),
	...
	if (HasConditions(bits_COND_HEAR_SOUND))
	{
		...
		if (pSound)
		{
			if (pSound->m_iType & bits_SOUND_DANGER)
			{
				...
				if (FOkToSpeak())
				{
					SENTENCEG_PlayRndSz(edict(), "HG_GREN", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
					JustSpoke();
				}
				return GetScheduleOfType( SCHED_TAKE_COVER_FROM_BEST_SOUND );
			}
			if (m_MonsterState == MONSTERSTATE_IDLE &&
				((pSound->m_iType & bits_SOUND_COMBAT) || (pSound->m_iType & bits_SOUND_DEATH)))// XDM: Advanced AI
			{
				if (FOkToSpeak())
				{
					SENTENCEG_PlayRndSz(edict(), "HG_HEAR", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
					JustSpoke();
				}
				return GetScheduleOfType( SCHED_GRUNT_CHECK_SOUND );
			}
	...
	case MONSTERSTATE_COMBAT:
		{
					...
					else
					{
						...
						if (FOkToSpeak())// && RANDOM_LONG(0,1))
						{
							if ((m_hEnemy != NULL) && m_hEnemy->IsPlayer())
								...
								// monster
								SENTENCEG_PlayRndSz( edict(), "HG_MONST", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
			...
			else if ( HasConditions( bits_COND_LIGHT_DAMAGE ) )// damaged just a little
			{
				...
				if ( iPercent <= 90 && m_hEnemy != NULL )
				{
					...
					if (FOkToSpeak()) // && RANDOM_LONG(0,1))
					{
						//SENTENCEG_PlayRndSz( edict(), "HG_COVER", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
			...
			else if (FBitSet(pev->weapons, HGRUNT_GRENADELAUNCHER) && HasConditions(bits_COND_CAN_RANGE_ATTACK2) && OccupySlot(bits_SLOTS_HGRUNT_GRENADE))// can launch grenade
			{
...
// can't see enemy
			else if ( HasConditions( bits_COND_ENEMY_OCCLUDED ) )
			{
				if ( HasConditions( bits_COND_CAN_RANGE_ATTACK2 ) && OccupySlot( bits_SLOTS_HGRUNT_GRENADE ) )
				{
					...
					if (FOkToSpeak())
					{
						SENTENCEG_PlayRndSz( edict(), "HG_THROW", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
						JustSpoke();
					}
					return GetScheduleOfType( SCHED_RANGE_ATTACK2 );
				}
				else if ( OccupySlot( bits_SLOTS_HGRUNT_ENGAGE ) )
				{
					...
					if (FOkToSpeak())// && RANDOM_LONG(0,1))
					{
						//SENTENCEG_PlayRndSz( edict(), "HG_CHARGE", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
				...
				else
				{
					...
					if (FOkToSpeak() && RANDOM_LONG(0,1))
					{
						SENTENCEG_PlayRndSz( edict(), "HG_TAUNT", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
						JustSpoke();
					}
...
Schedule_t *CHGrunt::GetScheduleOfType(int Type)
{
	...
	case SCHED_FAIL:
		{
			if (m_hEnemy != NULL)
				return &slGruntCombatFail[0];// grunt has an enemy, so pick a different default fail schedule most likely to help recover.
	...
	case SCHED_GRUNT_REPEL_LAND:
		...
		break;
	case SCHED_GRUNT_CHECK_SOUND:
		{// XDM
			return &slGruntCheckSound[0];
		}
		break;
	default:
		{
...
// XDM: throw a gun if the grunt has one
void CHGrunt::DropItems(void)
{
	if (GetBodygroup(HG_GUN_GROUP) == GUN_NONE)
		return;// XDM3038: already dropped

	Vector vecGunPos;
	Vector vecGunAngles;
	GetAttachment(0, vecGunPos, vecGunAngles);
	// switch to body group with no gun.
	SetBodygroup(HG_GUN_GROUP, GUN_NONE);
	...
	if (pGun)
	{
		...
		//pGun->pev->avelocity.Set(0.0f, RANDOM_FLOAT(200, 400), 0.0f);
	}
	...
	if (FBitSet(pev->weapons, HGRUNT_HANDGRENADE))// XDM3035a
	{
		...
		{
			pItem->pev->skin = m_iLastUsedGrenadeType;// XDM3035b
...
void CHGruntRepel::Precache(void)
{
	if (FStringNull(pev->message))
		pev->message = MAKE_STRING("monster_human_grunt");

	UTIL_PrecacheOther(STRING(pev->message));
...
{
	TraceResult tr;
	UTIL_TraceLine(pev->origin, pev->origin + Vector(0.0f, 0.0f, -4096.0f), dont_ignore_monsters, edict(), &tr);
	/*if (tr.pHit && tr.pHit->v.solid != SOLID_BSP)
		return NULL;*/

	CBaseEntity *pEntity = Create(STRING(pev->message), pev->origin, pev->angles);
	if (pEntity)
	{
		...
		{
			conprintf(2, "Warning: %s \"%s\": %s is not a monster!\n", STRING(pev->classname), STRING(pev->targetname), STRING(pev->message));
			UTIL_Remove(pGrunt);
		}
	}
	else
		conprintf(2, "Warning: %s \"%s\": failed to create %s!\n", STRING(pev->classname), STRING(pev->targetname), STRING(pev->message));

	Destroy();
}


// XDM: robo-grunt. Same as human, but it is CLASS_MACHINE and has some effects.
...
LINK_ENTITY_TO_CLASS( monster_robo_grunt, CRoboGrunt );
...
void CRoboGrunt::Spawn(void)
{
	...
	if (m_iScoreAward == 0)
		m_iScoreAward = gSkillData.rgruntScore;
...
{
	if (FStringNull(pev->model))// XDM3037
		pev->model = MAKE_STRING("models/rgrunt.mdl");

	PRECACHE_SOUND("rgrunt/rg_die.wav");
	//m_iGibs1 = PRECACHE_MODEL("models/metalplategibs_green.mdl");
	//m_iGibs2 = PRECACHE_MODEL("models/metalplategibs.mdl");
	m_iszGibModel = ALLOC_STRING("models/metalplategibs_green.mdl");
	m_iGibModelIndex = PRECACHE_MODEL(STRINGV(m_iszGibModel));
	CHGrunt::Precache();
}

bool CRoboGrunt::GibMonster(void)
{
	DropItems();// XDM
	PLAYBACK_EVENT_FULL(0, edict(), g_usSparkShower, 0, pev->origin, pev->angles, 200.0f, 1.0f, /*g_iModelIndexZeroFlare*/0, 4, 1, 0);
	//UTIL_Sparks(pev->origin + UTIL_RandomBloodVector());
	MESSAGE_BEGIN(MSG_PVS, svc_temp_entity, pev->origin);
		...
		WRITE_COORD(20);//size.z
		WRITE_COORD(8);//velocity.x
		WRITE_COORD(8);//velocity.y
		WRITE_COORD(32);//velocity.z
		WRITE_BYTE(10);//random velocity 0.1
		WRITE_SHORT(m_iGibModelIndex);
		WRITE_BYTE(16);//count
		WRITE_BYTE(2);//life 0.1
		WRITE_BYTE(BREAK_METAL|BREAK_SMOKE);//flags
	MESSAGE_END();
	/*MESSAGE_BEGIN(MSG_PVS, svc_temp_entity, pev->origin);
		WRITE_BYTE(TE_BREAKMODEL);
		WRITE_COORD(pev->origin.x);
		...
		WRITE_COORD(8);//velocity.y
		WRITE_COORD(32);//velocity.z
		WRITE_BYTE(2);//random velocity 0.1
		WRITE_SHORT(m_iGibs2);
		WRITE_BYTE(24);//count
...
void CRoboGrunt::PainSound(void)
{
	...
	{
		EMIT_SOUND_DYN(edict(), CHAN_VOICE, gSoundsMetal[RANDOM_LONG(0,NUM_SHARD_SOUNDS-1)], VOL_NORM, ATTN_NORM, 0, RANDOM_LONG(95,105));
...
{
	EMIT_SOUND(edict(), CHAN_VOICE, "rgrunt/rg_die.wav", VOL_NORM, ATTN_IDLE);
}

int CRoboGrunt::TakeDamage(CBaseEntity *pInflictor, CBaseEntity *pAttacker, float flDamage, int bitsDamageType)
{
	MESSAGE_BEGIN(MSG_PVS, svc_temp_entity, pev->origin);
		WRITE_BYTE(TE_SMOKE);
	...
	MESSAGE_END();
	return CHGrunt::TakeDamage(pInflictor, pAttacker, flDamage, bitsDamageType);
}

void CRoboGrunt::TraceAttack(CBaseEntity *pAttacker, float flDamage, const Vector &vecDir, TraceResult *ptr, int bitsDamageType)
{
	if (pev->takedamage != DAMAGE_NO && flDamage > 4.0f)
	{
		UTIL_Sparks(ptr->vecEndPos);
		//AddMultiDamage(pAttacker, this, flDamage, bitsDamageType);
	}
	CHGrunt::TraceAttack(pAttacker, flDamage, vecDir, ptr, bitsDamageType);// XDM3038c
}

//=========================================================
// DEAD HGRUNT PROP
//=========================================================
class CDeadHGrunt : public CHGrunt
{
public:
	virtual void Spawn(void);
...
void CDeadHGrunt::Spawn(void)
{
	...
	if (pev->health <= 0)
		pev->health = gSkillData.hgruntHealth * 0.6;// XDM
	...
	pev->deadflag		= DEAD_DEAD;// XDM3038c: required for Precache() and Killed()!
	Precache();
	SET_MODEL(edict(), STRING(pev->model));// XDM3037
	pev->effects			= 0;
	pev->yaw_speed		= 8;
	...
	if (pev->sequence == -1)
		conprintf(0, "Design error: %s[%d] with bad pose: %s!\n", STRING(pev->classname), entindex(), m_szPoses[m_iPose]);
	...
	case 0: // Grunt with Gun
		pev->body = 0;
		pev->skin = 0;
		SetBodygroup(HG_HEAD_GROUP, HEAD_GRUNT);
		SetBodygroup(HG_GUN_GROUP, GUN_MP5);
		break;
	case 1: // Commander with Gun
		pev->body = 0;
		pev->skin = 0;
		SetBodygroup(HG_HEAD_GROUP, HEAD_COMMANDER);
		SetBodygroup(HG_GUN_GROUP, GUN_MP5);
		break;
	case 2: // Grunt no Gun
		pev->body = 0;
...
{
	CHGrunt::Precache();// XDM3038c: !!! need to precache drop item too!
