...
//=========================================================
enum houndeye_animation_events_e
{
	HOUND_AE_WARN = 1,
	HOUND_AE_STARTATTACK,
	HOUND_AE_THUMP,
	HOUND_AE_ANGERSOUND1,
	HOUND_AE_ANGERSOUND2,
	HOUND_AE_HOPBACK,
	HOUND_AE_CLOSE_EYE
};
...
const char *CHoundeye::pDeathSounds[] = 
{
...
void CHoundeye :: Spawn()
{
	...
	if (m_iScoreAward == 0)
		m_iScoreAward = gSkillData.houndeyeScore;
	if (m_iGibCount == 0)
		m_iGibCount = 6;// XDM: small one
	if (m_flDamage1 == 0)// XDM3038c
		m_flDamage1 = gSkillData.houndeyeDmgBlast;
...
{
	for (size_t i = 0; i < ARRAYSIZE(sHoundHints); ++i)
	{
		if (sHoundHints[i] == sHint)
			return true;
	}
	return CSquadMonster::FValidateHintType(sHint);// XDM3038c
}
...
bool CHoundeye :: FCanActiveIdle (void)
{
	if ( InSquad() )
	{
		...
		for (short i = 0; i < MAX_SQUAD_MEMBERS;i++)
		{
...
void CHoundeye :: SetYawSpeed (void)
{
	...
	pev->yaw_speed = ys * gSkillData.iSkillLevel;// XDM3035c
}
...
void CHoundeye :: HandleAnimEvent(MonsterEvent_t *pEvent)
{
	switch ( pEvent->event )
	{
		...
		case HOUND_AE_HOPBACK:
			{
				float flGravity = g_psv_gravity?g_psv_gravity->value:DEFAULT_GRAVITY;
		...
		case HOUND_AE_ANGERSOUND1:
			EMIT_SOUND(edict(), CHAN_VOICE, pPainSounds[2], GetSoundVolume(), ATTN_NORM);	
			break;

		case HOUND_AE_ANGERSOUND2:
			EMIT_SOUND(edict(), CHAN_VOICE, pPainSounds[0], GetSoundVolume(), ATTN_NORM);	
			break;
...
{
	EMIT_SOUND_ARRAY_DYN(CHAN_VOICE, pHuntSounds);// XDM3038c
}
...
{
	EMIT_SOUND(edict(), CHAN_WEAPON, RANDOM_SOUND_ARRAY(pBlastSounds), VOL_NORM, ATTN_NORM);

	if (!g_pGameRules || g_pGameRules->FAllowEffects())// XDM
	{
	MESSAGE_BEGIN(MSG_PVS, svc_temp_entity, pev->origin);
		...
		WRITE_BYTE(HOUNDEYE_MAX_ATTACK_RADIUS * 0.1);
		WriteBeamColor();
		WRITE_BYTE(10);
		WRITE_BYTE(160);
	MESSAGE_END();
	...
	if (SquadCount() > 1)// squad gets attack bonus.
		flAdjustedDamage = m_flDamage1 + m_flDamage1 * ( HOUNDEYE_SQUAD_BONUS * ( SquadCount() - 1 ) );
	else// solo
		flAdjustedDamage = m_flDamage1;

	::RadiusDamage(pev->origin, this, this, flAdjustedDamage, HOUNDEYE_MAX_ATTACK_RADIUS, CLASS_NONE, DMG_SONIC | DMG_ALWAYSGIB | DMG_WALLPIERCING);
}
...
// XDM
int CHoundeye::TakeDamage(CBaseEntity *pInflictor, CBaseEntity *pAttacker, float flDamage, int bitsDamageType)
{
	if (FBitSet(bitsDamageType, DMG_SONIC) && IsAlive())// XDM3038c
		return 0;
