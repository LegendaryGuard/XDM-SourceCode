...
void CFlyingMonster::Killed(CBaseEntity *pInflictor, CBaseEntity *pAttacker, int iGib)
{
	...
	pev->angles.z = 0;
	pev->angles.x = 0;
	CBaseMonster::Killed(pInflictor, pAttacker, iGib);// XDM3037: fix
}
...
void CFlyingMonster :: HandleAnimEvent(MonsterEvent_t *pEvent)
{
	...
	case FLYING_AE_FLAPSOUND:
		if ( m_pFlapSound )
			EMIT_SOUND( edict(), CHAN_BODY, m_pFlapSound, GetSoundVolume(), ATTN_NORM );	
		break;
...
BOOL CFlyingMonster::ProbeZ( const Vector &position, const Vector &probe, float *pFraction)
{
	...
	vec_t ProbeLength = (probe-position).Length();
	vec_t maxProbeLength = ProbeLength;
	vec_t minProbeLength = 0;

	vec_t diff = maxProbeLength - minProbeLength;
	while (diff > 1.0)
	{
		vec_t midProbeLength = minProbeLength + diff/2.0;
