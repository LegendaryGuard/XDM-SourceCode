...
{
	if (m_hValue.Get() == NULL)
		return NULL;
	else if (pFrom == NULL || (OFFSET(m_hValue->edict()) > OFFSET(pFrom->edict())))
	{
...
///		((CBaseAnimating *)pSubject)->GetAttachment( 0, vecPosition, vecJunk );
	...
	case 8:
		...
		return vecOffset + vecPosition;
	case 9:
		return vecOffset + pSubject->pev->origin + RandomVector(pSubject->pev->mins, pSubject->pev->maxs);
/*			Vector(
...
Vector CCalcAngleTransform::CalcVelocity( CBaseEntity *pLocus ) {
	...
	switch (pev->impulse) {														// impulse: choose output axis
		case 0:vecOutput.Set(transformation, 0, 0);break;
		case 1:vecOutput.Set(0, transformation, 0);break;
		case 2:vecOutput.Set(0, 0, transformation);break;
...
//	UTIL_MakeVectors(vecAngles); don't you *** touch any globals!
	Vector fw;
	AngleVectors(vecAngles, fw, NULL, NULL);
...
void CLocusVariable::Use(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value)
{
	...
	if (m_iszTargetName)
	{
		CMark *pMark = GetClassPtr((CMark *)NULL, "mark");
...
void CLocusBeam::Use(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value)
{
	...
	case 3: // point & offset
		...
		break;
	default:
		ALERT(at_console, "%s[%d] %s bad type %d!\n", STRING(pev->classname), entindex(), STRING(pev->targetname), pev->impulse);
		pBeam = NULL;
		break;
	}
	if (pBeam)
