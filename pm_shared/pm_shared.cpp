...
#include "extdll.h"
#include "util.h"
//#include "cbase.h"// g_pWorld
//#include "globals.h"// g_pWorld
#endif
...
extern unsigned short g_usPM_Longjump;// XDM3035b
...
DLL_GLOBAL playermove_t *pmove = NULL;// because VALVe shoved important API up this structure, we have to make it global
bool g_onladder = 0;
...
// Texture names
static size_t gcTextures = 0;
static char grgszTextureName[CTEXTURESMAX][CBTEXTURENAMEMAX];
static char grgchTextureType[CTEXTURESMAX];
//static char grgszCustomTextureName[CCUSTOMTEXTURESMAX][CBTEXTURENAMEMAX];
//static char grgchCustomTextureType[CCUSTOMTEXTURESMAX];

char *g_MainMaterialFileDir = "sound";
char *g_MainMaterialFileName = "materials.txt";

void PM_SwapTextures(const int &i, const int &j)
{
	static char chTemp;
	static char szTemp[ CBTEXTURENAMEMAX ];
...
// Purpose: 
// Input  : *name - 
// Output : size_t array index, CTEXTURESMAX on error
//-----------------------------------------------------------------------------
static size_t PM_FindTextureTypeIndex(const char *name)
{
	if (gcTextures > 0)
	{
	...
	int val;
	left = 0;
	right = gcTextures - 1;// things go bad @gcTextures == 124
...
{
	size_t index = PM_FindTextureTypeIndex(name);
	if (index < CTEXTURESMAX)
		return grgchTextureType[index];

	return CHAR_TEX_DEFAULT;
}

//-----------------------------------------------------------------------------
// Purpose: 
// Input  : *fullname - raw name with all prefixes
// Output : char
//-----------------------------------------------------------------------------
char PM_FindTextureTypeFull(const char *fullname)
{
	//char *pTextureName = fullname;
	size_t offset = 0;
	...
	/*if (offset == 0)// no modifications were made
		return PM_FindTextureType(fullname);

	char szbuffer[CBTEXTURENAMEMAX];
	strcpy(szbuffer, fullname+offset);// nope strncpy(szbuffer, fullname+offset, CBTEXTURENAMEMAX);
	szbuffer[CBTEXTURENAMEMAX-1] = 0;
	return PM_FindTextureType(szbuffer);*/
	return PM_FindTextureType(fullname+offset);
}

//-----------------------------------------------------------------------------
// Purpose: Load data from materialfile
//-----------------------------------------------------------------------------
void PM_InitTextureTypes(char *materialfile, bool clear)
{
	char buffer[512];
	...
	memset(buffer, 0, 512);
	// XDM3038c: why??? fileSize = pmove->COM_FileSize(materialfile);
	pMemFile = pmove->COM_LoadFile(materialfile, 5, &fileSize);
	if (!pMemFile || fileSize <= 1)
	{
		pmove->Con_DPrintf("PM_InitTextureTypes(%s): unable to load file.\n", materialfile);
		return;
	}
	filePos = 0;
	// for each line in the file...
	// XDM3038c: while (pmove->memfgets(pMemFile, fileSize, &filePos, buffer, 511) != NULL && (gcTextures < CTEXTURESMAX))
	while (memfgets(pMemFile, fileSize, filePos, buffer, 511) != NULL && (gcTextures < CTEXTURESMAX))
	{
		...
		// we copied name into array, but until we increment global counter it doesn't count
		found_index = PM_FindTextureTypeIndex(grgszTextureName[gcTextures]);
		if (found_index < CTEXTURESMAX)// duplicate found!
		{
			pmove->Con_DPrintf("PM_InitTextureTypes(%s): overriding \"%s\" '%c' -> '%c'\n", materialfile, grgszTextureName[gcTextures], (int)grgchTextureType[found_index], (int)cCurrentType);
			grgchTextureType[gcTextures] = cCurrentType;
		}
		else
		{
	...
	pmove->Con_DPrintf("PM_InitTextureTypes(%s): %u materials parsed (max %d)\n", materialfile, gcTextures, CTEXTURESMAX);
...
//			&fvol - 
//-----------------------------------------------------------------------------
static void PM_PlayStepSound(const int &step, const float &fvol)
{
	...
	if (gStepSounds[step] == NULL)// XDM3037
		return;
	...
	/* XDM3038c: BAD: prevents sounds in ducts
	...
	pmove->PM_PlaySound(CHAN_BODY, gStepSounds[step][pmove->RandomLong(0,1) + (pmove->iStepLeft * 2)], fvol, ATTN_NORM, 0, PITCH_NORM);// XDM3035a: :p
}

//-----------------------------------------------------------------------------
// Purpose: Determine texture info for the texture we are standing on.
//-----------------------------------------------------------------------------
static void PM_CatagorizeTextureType(void)
{
	//vec3_t start, end;
	Vector end;
	...
	pTextureName = pmove->PM_TraceTexture( pmove->onground, pmove->origin, end );
	if ( !pTextureName )
		return;
...
// Output : qboolean
//-----------------------------------------------------------------------------
static qboolean PM_AddToTouched(pmtrace_t tr, vec3_t &impactvelocity)
{
	int i;
...
//-----------------------------------------------------------------------------
static void PM_CheckVelocity(void)
{
//
// bound velocity
//
...
//-----------------------------------------------------------------------------
int PM_ClipVelocity(const vec3_t &in, const vec3_t &normal, float *out, const float &overbounce)
{
	float backoff;
	float angle = normal[2];
	int i, blocked = 0x00;// Assume unblocked.
	...
	for (i=0; i<3; ++i)
	{
		out[i] = in[i] - normal[i]*backoff;// in[] - change[]
...
//-----------------------------------------------------------------------------
// Purpose: used by PM_Physics_Toss
//-----------------------------------------------------------------------------
/*void PM_AddGravity(void)
{
	...
	if (pmove->waterlevel > 1)// XDM3037a: float
	{
		ent_gravity *= 0.5f;
		if (pmove->flags & FL_FLOAT)
			pmove->velocity[2] += (ent_gravity * pmove->movevars->gravity*0.2 * pmove->frametime);
	}
	else
...
void PM_AddCorrectGravity(void)
{
	...
	if (pmove->waterlevel > 1)// XDM3038: float
	{
		ent_gravity *= 0.5f;// slow down in water (imitate density)
		if (pmove->flags & FL_FLOAT)
			ent_gravity = -ent_gravity;// float upwards
	}
	// XDM3038a: useless because we don't have access to all entities physics :(
/*	float world_gravity;
	if (g_pWorld)
	{
#if defined (CLIENT_DLL)
		world_gravity = g_pWorld->baseline.gravity;
#else
		world_gravity = g_pWorld->pev->gravity;
#endif
	}
	pmove->velocity[2] -= (world_gravity * ent_gravity * pmove->movevars->gravity * pmove->frametime * 0.5f);*/
...
// Purpose: 
//-----------------------------------------------------------------------------
static void PM_FixupGravityVelocity(void)
{
	if (pmove->waterjumptime)
		return;
...
static int PM_FlyMove(void)
{
	...
	for (bumpcount=0; bumpcount<numbumps; ++bumpcount)
	{
		...
		trace = pmove->PM_PlayerTrace(pmove->origin, end, PM_NORMAL, PHYSENT_NONE);
...
// XDM3037a: bad place	if (pmove->dead)
//		return;
	...
	pmove->velocity[0] += accelspeed * wishdir[0];
	pmove->velocity[1] += accelspeed * wishdir[1];
	pmove->velocity[2] += accelspeed * wishdir[2];
}
...
//	int i;
	...
	wishvel[1] = pmove->forward[1]*fmove + pmove->right[1]*smove;
...
// first try just moving to the destination
	...
	trace = pmove->PM_PlayerTrace(pmove->origin, dest, PM_NORMAL, PHYSENT_NONE);
...
// apply ground friction
	if (pmove->onground != -1)  // On an entity that is the ground
	{
		...
		trace = pmove->PM_PlayerTrace(start, stop, PM_NORMAL, PHYSENT_NONE);
...
//			accel - 
//-----------------------------------------------------------------------------
static void PM_AirAccelerate(const Vector &wishdir, const float &wishspeed, const float &accel)
{
//	int			i;
...
//	speed = VectorNormalize(temp);
	...
	if (speed > 0)
	{
/* The idea is brilliant, but is not easy to do: player gets stuck
		if (speed <= PLAYER_MAX_WALK_SPEED/4)// XDM3037: must be synchronized with CBasePlayer::SetAnimation()!
			pmove->usehull = HULL_PLAYER_STANDING;// Hover animation is almost vertical.
		else
			pmove->usehull = HULL_PLAYER_CROUCHING;// While swimming, body is oriented horizontally. Emulate it!
*/
...
//
	...
	if (addspeed > 0)
	{
		...
		pmove->velocity[0] += accelspeed * wishvel[0];
		pmove->velocity[1] += accelspeed * wishvel[1];
		pmove->velocity[2] += accelspeed * wishvel[2];
	}
...
// assume it is a stair or a slope, so press down from stepheight above
	...
	trace = pmove->PM_PlayerTrace(start, dest, PM_NORMAL, PHYSENT_NONE);
...
// Output : qboolean
//-----------------------------------------------------------------------------
static qboolean PM_CheckWater(void)
{
	vec3_t	point;
	int		cont;
	int		truecont;
...
//			pmove->waterlevel = 2;
			...
			if (cont <= CONTENTS_WATER && cont > CONTENTS_TRANSLUCENT)
				...
				pmove->waterlevel = 2;// XDM3037
		}
		else
			pmove->waterlevel = 1;// XDM3037
...
// see if standing on something solid
	...
	{
		// Try and move down.
		tr = pmove->PM_PlayerTrace(pmove->origin, point, PM_NORMAL, PHYSENT_NONE);
...
// Output : int
//-----------------------------------------------------------------------------
static int PM_GetRandomStuckOffsets(const int &nIndex, const int &server, const Vector &offset)
{
...
//			&server - 
//-----------------------------------------------------------------------------
static void PM_ResetStuckOffsets(const int &nIndex, const int &server)
{
...
static void PM_SpectatorMove(void)
{
	...
	if (pmove->iuser1 == OBS_ROAMING || pmove->iuser1 == OBS_MAP_FREE)// XDM3037a
	{
#if defined (CLIENT_DLL)
...
//		PM_NoClip();?
		...
		wishvel[0] = pmove->forward[0]*fmove + pmove->right[0]*smove;
		wishvel[1] = pmove->forward[1]*fmove + pmove->right[1]*smove;
		wishvel[2] = pmove->forward[2]*fmove + pmove->right[2]*smove + pmove->cmd.upmove;
...
// Output : float
//-----------------------------------------------------------------------------
static float PM_SplineFraction(const float &inputvalue, const float &scale)
{
...
// Input  : &direction - 
//-----------------------------------------------------------------------------
static void PM_FixPlayerCrouchStuck(const int &direction)
{
...
static void PM_UnDuck(void)
{
	...
	{
		newOrigin.x += (pmove->player_mins[1].x - pmove->player_mins[0].x);
		newOrigin.y += (pmove->player_mins[1].y - pmove->player_mins[0].y);
		newOrigin.z += (pmove->player_mins[1].z - pmove->player_mins[0].z);
	}

	trace = pmove->PM_PlayerTrace(newOrigin, newOrigin, PM_NORMAL, PHYSENT_NONE);

	if ( !trace.startsolid )
	{
		pmove->usehull = HULL_PLAYER_STANDING;
		...
		if ( trace.startsolid )
		{
			...
			pmove->usehull = HULL_PLAYER_CROUCHING;
			return;
		}
...
// Input  : *pLadder - 
//-----------------------------------------------------------------------------
static void PM_LadderMove(physent_t *pLadder)
{
	vec3_t		ladderCenter;
	trace_t		trace;
	qboolean	onFloor;
...
// Output : physent_t
//-----------------------------------------------------------------------------
static physent_t *PM_Ladder(void)
{
	int			i;
	physent_t	*pe;
	hull_t		*hull;
...
// Purpose: WaterJump
//-----------------------------------------------------------------------------
static void PM_WaterJump(void)
{
	if (pmove->waterjumptime > 10000)
		pmove->waterjumptime = 10000;
	else if (pmove->waterjumptime == 0)
		return;
...
// Output : pmtrace_t
//-----------------------------------------------------------------------------
static pmtrace_t PM_PushEntity(const Vector &push)
{
	pmtrace_t	trace;
	vec3_t	end;
...
//-----------------------------------------------------------------------------
static void PM_Physics_Toss(void)
{
	pmtrace_t trace;
	vec3_t	move;
	float	backoff;
...
// add gravity
	if (pmove->movetype != MOVETYPE_FLY &&
		...
		PM_AddCorrectGravity();// XDM3038: was PM_AddGravity ();
...
//  movement logic does.
//-----------------------------------------------------------------------------
static void PM_PreventMegaBunnyJumping(void)
{
...
static void PM_Jump(void)
{
	...
	{	// swimming, not jumping
		...
		if ( pmove->flSwimTime <= 0 )
		{
			...
			pmove->PM_PlaySound(CHAN_BODY, gStepSounds[STEP_WADE][pmove->RandomLong(0,NUM_STEP_SOUNDS-1)], VOL_NORM, ATTN_NORM, 0, pmove->RandomLong(95,105));// XDM3035a
		}
		return;
	}
	...
	switch (pmove->RandomLong(0,1))// Note that CHAN_BODY is occupied by step sounds!
	{
	...
	if ((pmove->bInDuck) || (pmove->flags & FL_DUCKING))
	{
		...
		bool cansuperjump = atoi(pmove->PM_Info_ValueForKey(pmove->physinfo, PHYSKEY_LONGJUMP)) > 0;
		// Adjust for super long jump module
		// UNDONE -- note this should be based on forward angles, not current velocity.
		if (cansuperjump && (pmove->cmd.buttons & IN_DUCK) && (pmove->flDuckTime > 0) && Length(pmove->velocity) > 50.0f && DotProduct(pmove->velocity, pmove->forward) >= 0.95f)// XDM3037: velocity and LOS facing almost same direction
		{
			pmove->punchangle[0] = -5.0f;

			pmove->velocity[0] = pmove->forward[0] * PLAYER_LONGJUMP_SPEED * 1.6f;
			pmove->velocity[1] = pmove->forward[1] * PLAYER_LONGJUMP_SPEED * 1.6f;
			pmove->velocity[2] += sqrtf(2 * 800 * 56);

#if defined (CLIENT_DLL)// if this is a client DLL code, play event locally only (so server won't have to send to me too)
			// XDM3037a: TESTME!! Unpredictable things happen here! PM code is a pile of %s...
			CL_PlaybackEventDirect(FEV_HOSTONLY|FEV_UPDATE|FEV_CLIENT, pmove->player_index+1, EV_PM_Longjump, (pmove->flags & FL_DUCKING), pmove->origin, pmove->angles, 0, 0, pmove->waterlevel, 0, (pmove->onground == -1)?0:1, g_onladder);
//no event index			pmove->PM_PlaybackEventFull(FEV_HOSTONLY|FEV_CLIENT, pmove->player_index, g_usPM_Longjump, 0.0f, pmove->origin, pmove->angles, 0, 0, pmove->waterlevel, 0, (pmove->onground == -1)?0:1, g_onladder);
#else
// fail		pmove->PM_PlaybackEventFull(FEV_UPDATE, pmove->player_index, g_usPM_Longjump, 0.0f, pmove->origin, pmove->angles, 0, 0, pmove->waterlevel, 0, (pmove->onground == -1)?0:1, g_onladder);
			PLAYBACK_EVENT_FULL(FEV_SERVER|FEV_UPDATE, UTIL_ClientEdictByIndex(pmove->player_index+1), g_usPM_Longjump, 0.0f, pmove->origin, pmove->angles, 0, 0, pmove->waterlevel, 0, (pmove->onground == -1)?0:1, g_onladder);
#endif
		}
		else
...
static void PM_CheckWaterJump(void)
{
	...
	if (!(pmove->cmd.buttons & IN_JUMP))
		return;// XDM3038c: sometimes auto-jumping is VERY annoying
	...
	pmove->usehull = HULL_POINT;
	tr = pmove->PM_PlayerTrace(vecStart, vecEnd, PM_NORMAL, PHYSENT_NONE);
...
// Purpose: 
//-----------------------------------------------------------------------------
static void PM_CheckFalling(void)
{
	if (pmove->flFallVelocity <= 0.0f)
		return;

	if (pmove->flTimeStepSound >= 600)// XDM3038: HACK: special number. reduce number of sounds
		return;

	if (pmove->effects & EF_NODRAW)// XDM3038: gibbed/disintegrated
		return;

	if (pmove->onground != -1 || g_onladder || pmove->waterlevel >= 2)
	{
//strangely enough, sounds won't be played if issued only on server side! #if !defined (CLIENT_DLL)
		...
		pmove->flTimeStepSound = 1000;// XDM3038: reduce number of sounds
		// land sounds
		if (pmove->flFallVelocity > PLAYER_MAX_SAFE_FALL_SPEED)
		{
			if (pmove->waterlevel > 0)// XDM: most painful sound or big water splash
				...
				pmove->PM_PlaySound(CHAN_STATIC, "common/bodydrop4.wav", 1.0f, ATTN_NORM, 0, PITCH_NORM);
			else
				pmove->PM_PlaySound(CHAN_STATIC, "player/pl_fallpain3.wav", 1.0f, ATTN_NORM, 0, PITCH_NORM);

			//fvol = 1.0;
		}
		else if (pmove->flFallVelocity > PLAYER_MAX_SAFE_FALL_SPEED * 0.75f)
		{
			if (pmove->waterlevel > 0)// XDM: less painfull sound and little splash
				pmove->PM_PlaySound(CHAN_STATIC, "common/watersplash.wav", 0.5f, ATTN_STATIC, 0, 110);
			else if (pmove->dead)
				pmove->PM_PlaySound(CHAN_STATIC, "common/bodydrop3.wav", 1.0f, ATTN_NORM, 0, PITCH_NORM);
			else
				pmove->PM_PlaySound(CHAN_STATIC, "player/pl_fallpain2.wav", 0.8f, ATTN_STATIC, 0, PITCH_NORM);

			//fvol = 0.85;
		}
		else if (pmove->flFallVelocity > PLAYER_MAX_SAFE_FALL_SPEED * 0.5f)// XDM3035a
		{
			if (pmove->waterlevel > 0)
				pmove->PM_PlaySound(CHAN_STATIC, "common/watersplash.wav", 0.3f, ATTN_IDLE, 0, PITCH_NORM);
			else if (pmove->dead)
				pmove->PM_PlaySound(CHAN_STATIC, "common/bodydrop1.wav", 1.0f, ATTN_NORM, 0, PITCH_NORM);
			else
				pmove->PM_PlaySound(CHAN_STATIC, "player/pl_fallpain1.wav", 0.6f, ATTN_IDLE, 0, PITCH_NORM);// XDM: land sound
		...
		if (pmove->flFallVelocity > PLAYER_MAX_SAFE_FALL_SPEED * 0.5f)// don't play everytime!
		{
			...
			eargs.flags = FEV_HOSTONLY | FEV_CLIENT;
			eargs.entindex = pmove->player_index+1;
			...
			*/
			CL_PlaybackEventDirect(FEV_HOSTONLY|FEV_UPDATE|FEV_CLIENT, pmove->player_index+1, EV_PM_Fall, (pmove->flags & FL_DUCKING), pmove->origin, pmove->angles, pmove->flFallVelocity, fvol, pmove->waterlevel, 0, (pmove->onground == -1)?0:1, g_onladder);
			//pmove->runfuncs = 1;
			// this shit ain't gonna work!			pmove->PM_PlaybackEventFull(test1->value, pmove->player_index, g_usPM_Fall, 0.0f, pmove->origin, pmove->angles, pmove->flFallVelocity, fvol, pmove->waterlevel, pmove->dead, (pmove->onground == -1)?0:1, g_onladder);
#else
//wtf? This flag does nothing!!			flags = FEV_NOTHOST;
			// BUGBUG: how do I sent event? \(o_O)/ from server to everyone else in PVS except me?!
			// Here clientindex starts at 0 // Host_Error: EV_SV_Playback:  Client index 1 out of range
			//pmove->runfuncs = 1;
// same here!				pmove->PM_PlaybackEventFull(test1.value, pmove->player_index, g_usPM_Fall, 0.0f, pmove->origin, pmove->angles, pmove->flFallVelocity, fvol, pmove->waterlevel, pmove->dead, (pmove->onground == -1)?0:1, g_onladder);
			PLAYBACK_EVENT_FULL(FEV_SERVER|FEV_UPDATE, UTIL_ClientEdictByIndex(pmove->player_index+1), g_usPM_Fall, 0.0f, pmove->origin, pmove->angles, pmove->flFallVelocity, fvol, pmove->waterlevel, pmove->dead, (pmove->onground == -1)?0:1, g_onladder);
#endif
		}
...
// Output : float
//-----------------------------------------------------------------------------
static float PM_CalcRoll(const vec3_t &angles, const vec3_t &velocity, const float &rollangle, const float &rollspeed)
{
    float   sign;
    float   side;
    float   value;
...
// Input  : &punchangle - 
//-----------------------------------------------------------------------------
static void PM_DropPunchAngle(Vector &punchangle)
{
	//float len = VectorNormalize(punchangle);
	vec_t len = punchangle.NormalizeSelf();
...
// Purpose: 
//-----------------------------------------------------------------------------
static void PM_CheckParamters(void)
{
	float spd;
	float maxspeed;
	vec3_t	v_angle;
	...
	if ( pmove->flags & FL_FROZEN ||
		 ...
		 (pmove->dead && pmove->spectator == 0))// XDM3037a
	{
	...
	if ( !pmove->dead )
	{
		...
		// Set up view angles.
		pmove->angles[ROLL]	=	PM_CalcRoll ( v_angle, pmove->velocity, pmove->movevars->rollangle, pmove->movevars->rollspeed )*4.0f;
	...
	/* XDM3037: HAHA! Finalliy found it! Unhacked!	if ( pmove->dead )
	{
	...
	//NormalizeAngle180(&pmove->angles[YAW]);?
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
static void PM_ReduceTimers(void)
{
	if (pmove->flTimeStepSound > 0)
	{
...
// XDM3037a: WARNING: spectator has ->dead and ->deadflag set! But he still needs movement!
=============
*/
static void PM_PlayerMove(qboolean server)
{
	...
	{
		if (atoi(pmove->PM_Info_ValueForKey(pmove->physinfo, PHYSKEY_IGNORELADDER)) <= 0)// XDM3038a: ignore ladder
			pLadder = PM_Ladder();
...
//	PM_UpdateStepSound();
	...
	if ((pmove->onground != -1) && (pmove->cmd.buttons & IN_USE) && pmove->gravity > 0 && pmove->friction >= 1.0f)// XDM3038c: enough cheating! New concept: a player MAY stop quickly, but only when fricion is sufficent
	...
	case MOVETYPE_WALK:
		...
		if ((pmove->oldwaterlevel == 0) != (pmove->waterlevel == 0))// XDM3037a: bool trick :)
			pmove->PM_PlaySound(CHAN_BODY, gStepSounds[STEP_WADE][pmove->RandomLong(0,NUM_STEP_SOUNDS-1)], VOL_NORM, ATTN_NORM, 0, pmove->RandomLong(95,105));// XDM3035a
...
// Purpose: 
//-----------------------------------------------------------------------------
static void PM_CreateStuckTable(void)
{
	float x, y, z;
	int idx;
	int i;
...
void PM_Move ( struct playermove_s *ppmove, int server )
{
	...
	if (pmove->flags & FL_SPECTATOR)
		pmove->spectator = 1;// XDM3037a: WTF WHY IS IT ALWAYS 0?!!

	if (pmove->deadflag > DEAD_NO)// && pmove->spectator == 0)
	{
		//happens often		ASSERT(pmove->dead > 0);
		pmove->dead = 1;// XDM3037a: a little hack
	}
...
//-----------------------------------------------------------------------------
// Purpose: XDM Special: find physent ID for specified entity index. Another BS.
// Input  : entindex - 
// Output : int
//-----------------------------------------------------------------------------
int PM_GetPhysent(int entindex)
{
	...
	for (pei = 0; pei < pmove->numphysent; ++pei)
	{
		if (pmove->physents[pei].info == entindex)
		{
			pe = pei;
			break;
		}
...
//-----------------------------------------------------------------------------
// Purpose: For external use, the most complete trace function.
// Note   : This one CAN include or IGNORE local player and still hit other entities!
// Warning: traceFlags - PM_WORLD_ONLY, etc.
//-----------------------------------------------------------------------------
pmtrace_t PM_PlayerTraceEx(const vec3_t &start, const vec3_t &end, int traceFlags, int usehull, int (*pfnIgnore)(struct physent_s *pe))
{
	int savehull = pmove->usehull;
	pmove->usehull = usehull;
	const pmtrace_t &trace = pmove->PM_PlayerTraceEx(start, end, traceFlags, pfnIgnore);
	pmove->usehull = savehull;
	return trace;
}

//-----------------------------------------------------------------------------
// Purpose: For external use, less customizeable, but a little faster than PM_PlayerTraceEx
// Warning: flags - PM_TRACELINE_PHYSENTSONLY, etc.
//-----------------------------------------------------------------------------
pmtrace_t *PM_TraceLineEx(const vec3_t &start, const vec3_t &end, int flags, int usehull, int (*pfnIgnore)(struct physent_s *pe))
{
	return pmove->PM_TraceLineEx(start, end, flags, usehull, pfnIgnore);
}

//-----------------------------------------------------------------------------
// Purpose: Load global and optional per-map materials (overwrites globals)
// Input  : *mapname - optional map name, "subdir/mapname"
//-----------------------------------------------------------------------------
void PM_InitMaterials(const char *mapname)
{
	pmove->Con_DPrintf("PM_InitMaterials(%s) (server: %d)\n", mapname, pmove?pmove->server:-1);
	if (pmove == NULL)
		return;

	char matfile[128];// XDM3038c: how about "maps/CO_ArcticIncident02_abandoned_materials.txt"?
	_snprintf(matfile, 128, "%s/%s\0", g_MainMaterialFileDir, g_MainMaterialFileName);
	PM_InitTextureTypes(matfile, true);
	if (mapname && *mapname)
	{
		_snprintf(matfile, 128, "maps/%s_%s\0", mapname, g_MainMaterialFileName);
		PM_InitTextureTypes(matfile, false);// XDM3038a: overrides global materials (if any)
	}
}

//-----------------------------------------------------------------------------
// Purpose: PM_Init external function
// Input  : *ppmove - the API
//-----------------------------------------------------------------------------
