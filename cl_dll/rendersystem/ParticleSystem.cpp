...
void CParticleSystem::InitializeSystem(void)
{
	...
	CParticleSystem::BaseClass::InitializeSystem();// <- FollowUpdatePosition(); <- color and brightness

	// UNDONE: WTF: HELP: F1: Calculate average particle life
	if (m_fParticleLife <= 0.0f)// "auto"
	{
		// particle_life_Sec = (m_fColorCurrent[3]/m_fColorDelta[3]) // right now m_fColorCurrent[3] is set to initial start value
		// particle_spawn_delay_Sec = particle_life_Sec/m_iMaxParticles;
		if (m_fColorDelta[3] != 0.0f)// defines the number of iterations in the gradient // WARNING: this code does NOT handle situations where particle's initial brigtness is changed during system lifetime!!
		{
			float fBrightnessRange = (m_fColorDelta[3] < 0.0f) ? (m_fColorCurrent[3] - 0.0f) : (1.0f - m_fColorCurrent[3]);// range that actual brightness travels through
...
uint32 CParticleSystem::Emit(void)
{
	...
	{
		conprintf(3, "CParticleSystem::Emit(iNumEmit %u) warning: m_fAccumulatedEmit(%f) > m_iMaxParticles(%u)!\n", iNumEmit, m_fAccumulatedEmit, m_iMaxParticles);
...
void CParticleSystem::InitializeParticle(CParticle *pParticle, const float &fInterpolaitonMult)
{
	...
	{
		// no pParticle->m_vVel = vecRand; vecRand may be position in the box
		if (!m_vSpread.IsZero())
		{
			VectorRandom(pParticle->m_vVel, -1.0f, 1.0f);
	...
	else if (m_iMovementType == PSMOVTYPE_OUTWARDS)
	{
		...
		pParticle->m_vVel.NormalizeSelf();// sphere with uneven distribution
		if (!m_vSpread.IsZero())
	...
	// IMPORTANT: make sure m_vVel is set and valid by now!
	...
	if (m_OnParticleInitialize == NULL || m_OnParticleInitialize(this, pParticle, m_pOnParticleInitializeData, fInterpolaitonMult) == true)
	{
		...
		//pParticle->SetColor(m_color, m_fColorCurrent[3]);
...
void CParticleSystem::Render(void)
{
	...
	for (uint32 i = 0; i < m_iMaxParticles; ++i)// XDM3037: unstable m_iNumParticles; ++i)
	{
		...
		if (FBitSet(p->m_iFlags, PARTICLE_ORIENTED))// R&U in p->Render() must point along face/texture sides
		{
			...
			p->Render(v_right, v_up, m_iRenderMode);// m_iDoubleSided is pointless
	}
#endif
