...
int CHudGeiger::MsgFunc_Geiger(const char *pszName,  int iSize, void *pbuf)
{
	...
	m_iGeigerRange = (int)READ_BYTE() << 2;// XDM3038
	END_READ();
...
{
	// Ignore gHUD.m_iHideHUDDisplay!

	if (!gHUD.PlayerHasSuit())// XDM3038: should be checked on server
		return 0;
	...
	if (m_iGeigerRange < 1000 && m_iGeigerRange > 0)
	{
		...
		if (i>0 && pct>0 && ((rand() & 127) < pct || (rand() & 127) < pct))
		{
			...
			char sz[256];
			_snprintf(sz, 256, "player/geiger%d.wav", rg[RANDOM_LONG(0,i-1)]);
			PlaySound(sz, flvol);
		}
	}
	return 1;
